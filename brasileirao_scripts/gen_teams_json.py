...
Uso: python brasileirao_scripts/gen_teams_json.py   (após python -m brasileirao_predictor.ingest_sofascore)
"""
...
def main() -> None:
    ...
    if not files:
        sys.exit(f"cache vazio ({pattern}) — rode python -m brasileirao_predictor.ingest_sofascore antes")
    for f in files:
