...
O banco é criado VAZIO, com o schema canônico — `brasileirao_predictor.db.connect` já executa o
...
Uso:
    python brasileirao_scripts/seed_test_fixtures.py           # cria só o que falta
    python brasileirao_scripts/seed_test_fixtures.py --force   # recria (DESTRUTIVO)
"""
...
def build_database(path: Path) -> None:
    """Cria o banco vazio com o schema canônico de `brasileirao_predictor.db`."""
    sys.path.insert(0, str(ROOT))
    from brasileirao_predictor.db import connect  # importado aqui: precisa do ROOT no sys.path
