...
bands = [(0, 0.20), (0.20, 0.23), (0.23, 0.26), (0.26, 0.30), (0.30, 1)]
brier_d = 0
for lo, hi in bands:
    g = [p for p in pts if lo <= p[0] < hi]
    if not g:
        continue
    pv = st.mean(x[0] for x in g)
    ob = st.mean(x[1] for x in g)
...
print(
    f"  jogos: {len(mpts)} | P(empate) mercado medio: {st.mean(x[0] for x in mpts):.1%} | "
