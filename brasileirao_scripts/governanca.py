...
Uso: python brasileirao_scripts/governanca.py
"""
...
from predictor_core.measurement.trials import TrialRegistry, attestation_path_for  # noqa: E402
from predictor_core.testing.harness import attest_pipeline_power  # noqa: E402

from brasileirao_predictor import db, model, ratings  # noqa: E402
...
def _make_series(params, cfg, *, inflated: bool, seed: int):
    ...
    for _ in range(N_GAMES):
        ...
        for sel, sel_p_model, p_mkt in (
            ("over", p_model, p_over_base),
...
def evaluate_funnel(returns):
    ...
    if len(returns) < 30:
        return {"verdict": "REFUTADA", "n": len(returns), "motivo": "amostra insuficiente"}
    psr = probabilistic_sharpe_ratio(returns, 0.0)
    lo, hi, _ = bootstrap_ci(returns, st.mean, scheme="iid", n_boot=2000, seed=SEED)
