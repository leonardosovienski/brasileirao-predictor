...
from brasileirao_predictor.data.the_odds_api_provider import ADAPTER_VERSION, SOURCE, SPORT, TheOddsApiProvider


def report(rows: list[dict], region: str) -> dict:
    events = defaultdict(list)
    for row in rows:
        events[row["source_event_id"]].append(row)
    books = Counter(row["bookmaker"] for row in rows)
    return {
        "schema_version": "odds-source-smoke/v1",
        "source": SOURCE,
        "adapter_version": ADAPTER_VERSION,
        "sport": SPORT,
        "region": region,
        "events_with_ou25": len(events),
        "quotes_ou25": len(rows),
        "bookmakers": [
            {
                "key": key,
                "quotes": books[key],
                "events": len({r["source_event_id"] for r in rows if r["bookmaker"] == key}),
            }
            for key in sorted(books)
        ],
        "timestamps_utc": all(str(r["odds_captured_at"]).endswith("+00:00") for r in rows),
        "totals_present": bool(rows),
