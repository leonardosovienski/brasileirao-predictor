...
Install-PythonModuleTask -Name "brasileirao-model-update" -Module "brasileirao_predictor.cron_update_models" -Trigger $modelRefresh -Description "Atualiza cache Elo e parametros do serving" -TimeLimitMinutes 30
...
Install-ScriptTask -Name "brasileirao-h9-emit" -ScriptPath (Join-Path $repo "brasileirao_scripts\emit_h9_shadow.py") -Trigger $h9Emit -Description "H9 shadow: funil de decisao (15min)"
Install-ScriptTask -Name "brasileirao-h9-closing" -ScriptPath (Join-Path $repo "brasileirao_scripts\record_h9_closing_snapshots.py") -Trigger $h9Closing -Description "H9 shadow: snapshots de fechamento (15min)"
Install-ScriptTask -Name "brasileirao-h9-settle" -ScriptPath (Join-Path $repo "brasileirao_scripts\settle_h9_shadow.py") -Trigger $h9Settle -Description "H9 shadow: liquidacao pos-jogo (30min)"
