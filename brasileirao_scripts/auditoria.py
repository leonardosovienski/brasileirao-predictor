...
om = {}
for d, h, a_, oh, od, oa in conn.execute(
...
def ent(p):
    return -sum(x * math.log(max(x, 1e-9)) for x in p)


for i in wc:
    d, h, a_, hs, as_, t, neu = rows[i]
    r = model.predict_match(hist[i][0], 0.0, params, home_adv=0.0, max_goals=MAXG)
    lam_t.append(r["total_goals"])
    lam_d.append(r["lambda_a"] - r["lambda_b"])
    gols_reais.append(hs + as_)
    g = r["grid"]
    pdtot.append(r["p_draw"])
    pd00.append(float(g[0, 0]))
    pd11.append(float(g[1, 1]))
    pd22.append(float(g[2, 2]))
    pd33.append(float(g[3, 3]))
    pdrest.append(r["p_draw"] - float(g[0, 0] + g[1, 1] + g[2, 2] + g[3, 3]))
    pm = [r["p_win"], r["p_draw"], r["p_loss"]]
    Ph.append(pm[0])
    Pd.append(pm[1])
    Pa.append(pm[2])
    entropies_m.append(ent(pm))
    maxp_m.append(max(pm))
    o = odds(h, a_, d)
    if o:
        ...
        Md.append(sh[1])
        Ma.append(sh[2])
        entropies_k.append(ent(sh))
        maxp_k.append(max(sh))
...
print(
    f"  -> modelo {'SUBESTIMA' if st.mean(lam_t) < st.mean(gols_reais) else 'superestima'} "
...
flatness = "MAIS" if st.mean(entropies_m) > st.mean(entropies_k) else "menos"
print(f"  -> entropia MAIOR = mais 'achatado'. Modelo e {flatness} achatado que o mercado.")
