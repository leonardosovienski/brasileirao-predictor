...
Uso:
    python brasileirao_scripts/prever.py Spain Austria              # campo neutro (padrao Copa)
    python brasileirao_scripts/prever.py Brazil France --mando      # com vantagem de mando p/ o 1o time
    python brasileirao_scripts/prever.py Spain Austria --mata-mata  # inclui P(classificar)
    python brasileirao_scripts/prever.py Spain Austria --json       # machine-output
    python brasileirao_scripts/prever.py Argentina Egypt --primeiro-tempo
        # projecao do 1o tempo pre-jogo (SEM CLV validado)
    python brasileirao_scripts/prever.py Argentina Egypt --segundo-tempo 0-2
...
`src/display.py` — mesma fonte que `python -m brasileirao_predictor.predict` usa, sem
duplicação.

Read-only no banco. CLV histórico exibido vem do cache gravado por
`python -m brasileirao_predictor.bootstrap` (não é mais hardcoded no código-fonte).
"""
