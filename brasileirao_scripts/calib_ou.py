...
Uso:  python brasileirao_scripts/calib_ou.py   (a partir da raiz do repo)
"""
...
for d, h, a, oov, oun in conn.execute(
    ...
    "FROM sofascore_matches WHERE odds_over IS NOT NULL AND odds_under IS NOT NULL"
).fetchall():
...
for lo, hi in [(0, 0.10), (0.10, 0.20), (0.20, 0.30), (0.30, 0.45), (0.45, 1.01)]:
    g = [p for p in pairs if lo <= p[0] < hi]
    if not g:
        continue
    pm_ = st.mean(x[0] for x in g)
    re = st.mean(x[1] for x in g)
    flag = "super-confiante" if pm_ > re + 0.05 else ("sub-confiante" if re > pm_ + 0.05 else "ok")
...
for i in test_idx:
    ...
    mb.append(brier2(pov, y))
    kb.append(brier2(sh[0], y))
    ma.append(int((pov >= 0.5) == (y == 1)))
    ka.append(int((sh[0] >= 0.5) == (y == 1)))
...
if mb:
    ...
    print(
        f"  -> {'MODELO bate' if st.mean(mb) < st.mean(kb) else 'mercado bate'} (dif {st.mean(mb) - st.mean(kb):+.4f})"
    )
