...
def fit_maher(win, lr):
    ...
    return (
        res.x[0],
        res.x[1],
        dict(zip(tm, res.x[2 : 2 + N])),
        dict(zip(tm, res.x[2 + N : 2 + 2 * N])),
    )


def ml(par, g):
    mu, gam, atk, dfn = par
    return math.exp(mu + atk.get(g[1], 0) - dfn.get(g[2], 0) + (0 if g[6] else gam)), math.exp(
        mu + atk.get(g[2], 0) - dfn.get(g[1], 0)
    )
...
for y, m in months:
    ...
    for i, r in enumerate(rows):
        ...
        pM = gp(lh, la)
        d = hist[i][0] / 400.0
        pB = gp(math.exp(a_b + b_b * d), math.exp(a_b - b_b * d))
        bM, bB = brier(pM, y3), brier(pB, y3)
        diffs.append(bB - bM)  # >0 = Maher melhor
...
significance = "SIGNIFICATIVO" if abs(t) > 2 else "nao significativo"
print(f"  IC 95%: [{mean - 1.96 * se:+.4f}, {mean + 1.96 * se:+.4f}] -> {significance}")
...
for yr in sorted(by_year):
    g = by_year[yr]
    bm = st.mean(x[0] for x in g)
    bb = st.mean(x[1] for x in g)
