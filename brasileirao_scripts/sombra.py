...
Uso (rotina pré-rodada, depois do ingest):
    python -m brasileirao_predictor.ingest_sofascore                      # atualiza odds
    python brasileirao_scripts/sombra.py --capture                  # registra picks
    ... (jogos acontecem; novo ingest) ...
    python brasileirao_scripts/sombra.py --settle                   # liquida + CLV
    python brasileirao_scripts/sombra.py --report                   # painel da H3
...
from brasileirao_predictor.data.prospective_shadow import record_hash, validate_pick  # noqa: E402
...
def capture(cfg, conn) -> int:
    ...
    if not elo or not prow:
        sys.exit("cache vazio — rode python -m brasileirao_predictor.cron_update_models")
...
def capture_h5(cfg, conn) -> int:
    ...
    if not (cfg.get("ensemble_xg") or {}).get("enabled"):
        return 0
    from brasileirao_predictor import xg_model
