...
Uso:
    python brasileirao_scripts/ci_check.py            # tudo
    python brasileirao_scripts/ci_check.py --fast     # pula o pytest (so as barreiras estaticas)
...
# Smoke do serving no dominio Brasileirao: dois clubes que existem na base
# (6.2 do bootstrap). --mando porque liga nao tem campo neutro.
SMOKE_TEAMS = ("Flamengo", "Palmeiras")
...
RESEARCH_DB_FILES = [
    "brasileirao_predictor/backtest_event.py",
    "brasileirao_predictor/diagnose_event_data.py",
    "brasileirao_predictor/check_coverage.py",
    "brasileirao_predictor/research/verify_calibration.py",
    "brasileirao_predictor/research/test_combinations.py",
    "brasileirao_predictor/research/vorp_ridge.py",
    "brasileirao_predictor/research/survival_test.py",
    "brasileirao_scripts/sim_2025_2026.py",
    "brasileirao_scripts/sim_melhorias.py",
    "brasileirao_scripts/backtest_close.py",
]

# Modulos de SERVING: unicos autorizados a tocar current_elo/load_elo.
SERVING_ALLOWLIST = {
    "brasileirao_predictor/db.py",  # definicao do schema e load/save
    "brasileirao_predictor/cron_update_models.py",  # unico ESCRITOR do cache
    "brasileirao_predictor/predict.py",  # leitor de serving
    "brasileirao_predictor/simulator.py",  # leitor de serving
    "brasileirao_predictor/status.py",  # painel (leitura de contagens)
    "brasileirao_predictor/kernel_daemon.py",  # daemon de serving (v3)
    "brasileirao_scripts/odds_shop.py",  # serving pre-jogo (le o Elo corrente de proposito)
    "brasileirao_scripts/sombra.py",  # serving pre-jogo da H3: preve jogos FUTUROS com
...
KNOWN_DEBT = {
    "brasileirao_predictor/research/vorp_ridge.py",
    "brasileirao_predictor/research/survival_test.py",
    "brasileirao_scripts/diag_zebra.py",
}
...
def check_research_readonly() -> None:
    ...
    for rel in RESEARCH_DB_FILES:
        ...
        for m in re.finditer(r"db\.connect\s*\(", text):
            ...
            if "read_only=True" not in window:
                failures.append(
                    f"{rel}: db.connect sem read_only=True — pesquisa nao pode abrir o banco em modo escrita"
                )
    ...
    fb = ROOT / "brasileirao_predictor" / "feature_builder.py"
    if fb.exists() and "mode=ro" not in fb.read_text(encoding="utf-8", errors="replace"):
        failures.append("brasileirao_predictor/feature_builder.py: conexao sem mode=ro (P12 regrediu)")
...
def check_current_elo_containment() -> None:
    ...
    for f in files:
        ...
        if rel in SERVING_ALLOWLIST:
            continue
        code_hits = [ln for ln in text.splitlines() if usage.search(ln) and not ln.lstrip().startswith("#")]
        if not code_hits:
            continue
        if rel in KNOWN_DEBT:
            warnings_.append(
                f"{rel}: usa current_elo/load_elo (divida v3 conhecida "
                f"— corrigir com ratings_asof antes de usar em conclusao)"
            )
        else:
            failures.append(f"{rel}: current_elo/load_elo fora do serving — lookahead; use ratings.ratings_asof")
...
def check_predict_smoke() -> None:
    ...
    r = subprocess.run(
        [sys.executable, "-X", "utf8", "-m", "brasileirao_predictor.predict", *SMOKE_TEAMS, "--json"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    if not 99.0 <= total <= 101.0:
        failures.append(
            f"probabilidades 1X2 somam {total:.1f}% (esperado ~100%) — possivel regressao do P5 (max_goals/delta_vorp)"
        )
...
def _check_live_flag(flag: str, extra_args: list[str], result_key: str) -> None:
    ...
    r = subprocess.run(
        [sys.executable, "-X", "utf8", "brasileirao_scripts/prever.py", *SMOKE_TEAMS, *extra_args, "--json"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    out = r.stdout or ""
    if r.returncode != 0:
        failures.append(f"prever.py {flag} saiu com exit {r.returncode}: {(r.stderr or '')[-200:]}")
        return
    try:
        data = json.loads(out)
        ...
        failures.append(f"prever.py {flag} --json nao produziu o dict esperado ({e}) — schema de compute_live() mudou?")
        return
    total = (p_win + p_draw + p_loss) * 100
    if not 99.0 <= total <= 101.0:
        failures.append(
            f"{flag}: p_win+p_draw+p_loss ({result_key}) = {total:.1f}% "
            f"(esperado ~100%) — possivel regressao do compute_live"
        )
    print(f"      {flag} ({result_key}) = {p_win:.1%} / {p_draw:.1%} / {p_loss:.1%} (soma {total:.1f}%)")
