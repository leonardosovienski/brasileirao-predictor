...
from brasileirao_predictor.model import _nb_logpmf, _tau
...
hs = np.array([rows[i][3] for i in tr], float)
as_ = np.array([rows[i][4] for i in tr], float)
...
res = minimize(
    negll,
    [base, 0.3, -0.3, math.log(0.1), -0.03],
    method="L-BFGS-B",
    bounds=[(-3, 3), (-2, 4), (-4, 2), (math.log(1e-4), math.log(3)), (-0.4, 0.4)],
)
al0, b1, b2, la, rho_cf = res.x
alpha_cf = math.exp(la)
...
def evalu(name, fn):
    ...
    for i in ho:
        ...
        brs.append(brier(p, y))
        lls.append(-math.log(max(p[y], 1e-9)))
        mx.append(max(p))
        pd.append(p[1])
        obs.append(1 if y == 1 else 0)
    print(
        f"{name:<12} Brier {st.mean(brs):.4f} | LogL {st.mean(lls):.4f} | probMax {st.mean(mx):.1%} | "
...
evalu(
    "COSH-FREE",
    lambda dd: probs(math.exp(al0 + b1 * dd), math.exp(al0 + b2 * dd), alpha_cf, rho_cf),
)
