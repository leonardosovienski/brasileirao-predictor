...
def main():
    ...
    plus7 = (datetime.now(UTC) + timedelta(days=7)).strftime("%Y-%m-%d")
    fixtures = get("fixtures", sportId=10, **{"from": today, "to": plus7})
    br_fixtures = [f for f in fixtures if "brazil" in str(f.get("categorySlug", "")).lower() and f.get("hasOdds")]
    ...
    print(f"\n[3] Evento: {br_fixtures[0].get('participant1Name')} x {br_fixtures[0].get('participant2Name')}")
    ...
    for slug in ([REFERENCE] if has_pin else []) + present_br[:3]:
        ...
        def price(o):
            return m.get(o, {}).get("players", {}).get("0", {}).get("price", "-")
