...
def detect_ev(odds_pinnacle, odds_soft, book, event_id, market, selections,
              ev_threshold=0.03, max_staleness_s=300, ts_now=None, ts_pin=None,
              devig_method="shin"):
    ...
    for sel, o_soft, pf in zip(selections, odds_soft, p_fair):
        ...
        if ev > ev_threshold:
            out.append(EVAlert(event_id, market, sel, book, float(o_soft),
                               round(float(pf), 6), round(ev, 4),
                               ts_now.isoformat() if ts_now else "",
                               ts_pin.isoformat() if ts_pin else "", stale))
    return out
...
def detect_arb(odds_by_book, selections, min_profit=0.005):
    ...
    if inv < 1.0 - min_profit:
        return {"arb": True, "profit_pct": round((1.0/inv - 1.0)*100, 3),
                "inv_sum": round(inv, 6),
...
def dsr(trials, sr, n, skew=0.0, kurt=3.0):
    ...
    g = 0.5772156649
    sr_b = math.sqrt(var_sr) * ((1-g)*stats.norm.ppf(1-1/trials)
...
def roi_ic_bootstrap(retornos, n_boot=10000, seed=42):
    ...
    r = np.asarray(retornos, dtype=float)
    boots = np.array([rng.choice(r, size=len(r), replace=True).mean()
