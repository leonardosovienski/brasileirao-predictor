...
def fit(matches, xg_map, fit_date, cfg_xg=None):
    ...
    w = np.array([0.5 ** ((fd - date.fromisoformat(r[0][:10])).days / hl_days)
                  for r in matches])
    ...
    return {
        "mu": mu, "ha": ha,
        "alpha": float(math.exp(r2.x[0])), "rho": float(r2.x[1]),
        "atk": {t: float(atk[tix[t]]) for t in teams},
        "def": {t: float(dfn[tix[t]]) for t in teams},
        "fit_date": fit_date[:10], "n_matches": len(matches),
...
def predict(xgp, home, away, neutral=False, max_goals=12):
    ...
    return {"lambda_a": lam_h, "lambda_b": lam_a,
            "total_goals": lam_h + lam_a, **_grid_stats(grid, max_goals)}
...
def blend(r_base, r_xg, w_base=0.5):
    ...
    return {"lambda_a": lam_h, "lambda_b": lam_a,
            "total_goals": lam_h + lam_a, "ensemble": True,
...
def maybe_blend(r, conn, cfg, name_a, name_b, neutral):
    ...
    try:
        ...
        if not row:
            print("[ensemble_xg ligado mas sem cache — rode "
                  "`python -m src.cron_update_models`; usando baseline]",
                  file=sys.stderr)
            return r
        xgp = row[0]
