...
def log_prediction(
    ...
    record = {
        ...
        "elo_home": round(float(elo_home), 1),
        "elo_away": round(float(elo_away), 1),
        "lambda_home": round(pred["lambda_a"], 4),
        "lambda_away": round(pred["lambda_b"], 4),
        "total_goals": round(pred["total_goals"], 4),
        "p_home": round(pred["p_win"], 4),
        "p_draw": round(pred["p_draw"], 4),
        "p_away": round(pred["p_loss"], 4),
        "over": over,
        "under": {k: round(1.0 - v, 4) for k, v in over.items()},
        "btts_yes": round(pred["btts"], 4),
            ...
            record["market"].update(
                {
                    ...
                    "p_over": round(market["p_over"], 4),
                    "p_under": round(market["p_under"], 4),
...
def log_period_prediction(
    ...
    record = {
        ...
        if calibration is None
            ...
            "frac1": round(calibration["frac1"], 4),
            "n": calibration["n"],
            "ci_low": None if calibration.get("ci_low") is None else round(calibration["ci_low"], 4),
            "ci_high": None if calibration.get("ci_high") is None else round(calibration["ci_high"], 4),
        },
        "period_pred": {
            "lambda_home": round(per["lambda_a"], 4),
            "lambda_away": round(per["lambda_b"], 4),
            "p_home": round(per["p_win"], 4),
            "p_draw": round(per["p_draw"], 4),
            "p_away": round(per["p_loss"], 4),
            "over": {str(k): round(v, 4) for k, v in per["over"].items()},
        },
        "final_pred": {
            "p_home": round(fin["p_win"], 4),
            "p_draw": round(fin["p_draw"], 4),
