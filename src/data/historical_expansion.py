...
def ingest_api_football(conn: sqlite3.Connection, rows: Iterable[dict[str, Any]],
                        *, known_teams: set[str]) -> int:
    values = []
    for row in rows:
        ...
        values.append((
            row["source"], row["source_event_id"], int(row["season"]),
            row["scheduled_at"], row["scheduled_at"][:10],
            row["home_team"], row["away_team"], home, away,
            row.get("home_goals"), row.get("away_goals"), row.get("status"), 1,
        ))
    conn.executemany("""
...
def coverage_report(shadow: sqlite3.Connection,
                    production: sqlite3.Connection) -> dict[str, Any]:
    ...
    for date, home, away, hg, ag in shadow.execute(
        ...
        if current is None:
            ...
            score_conflicts += 1
    seasons = dict(shadow.execute(
        "SELECT season,count(*) FROM shadow_matches GROUP BY season ORDER BY season"))
    return {
        "total": total, "with_score": scored, "first_date": first_date,
        "last_date": last_date, "by_season": seasons,
        "exact_duplicates": exact_duplicates, "score_conflicts": score_conflicts,
