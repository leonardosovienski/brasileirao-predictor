...
class SportmonksProvider:
    ...
    def _headers(self) -> dict[str, str]:
        if not self.token:
            raise DataUnavailableError("SPORTMONKS_API_TOKEN ausente")
        # Sportmonks aceita o token bruto no Authorization; Bearer retorna 401.
        return {"Authorization": self.token, "User-Agent": "brasileirao-predictor-source-audit/1.0"}
    ...
    def require_league(self, name: str, country: str) -> int:
        matches = [row for row in self.accessible_leagues() if row["name"] == name and row["country"] == country]
        if len(matches) != 1:
            raise DataUnavailableError(f"liga não coberta pelo plano Sportmonks: {name} ({country})")
        return matches[0]["source_league_id"]

    def list_fixtures(
        self, *, league_id: int, from_date: date, to_date: date, observed_at: datetime | None = None
    ) -> list[dict[str, Any]]:
        ...
        for fixture in raw_rows:
            ...
            rows.append(
                {
                    "source": "sportmonks",
                    "source_event_id": str(fixture["id"]),
