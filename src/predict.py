...
def build(cfg):
    ...
    if not elo or not prow:
        print(
            "[cache vazio — calculando agora; rode `python -m src.cron_update_models` para tornar a CLI instantânea]",
            file=sys.stderr,
        )
...
def show(
    ...
    emit_event(
        ...
        "prediction",
        metrics={
            "home_goals_pred": round(float(r["lambda_a"]), 3),
    ...
    if not quiet:
        ...
        if not as_json and conn is not None:
            if corners:
                ev = display.compute_event(conn, elo, name_a, name_b, "Corner kicks", (7.5, 8.5, 9.5))
                display.render_event("Escanteios", ev, name_a, name_b, (7.5, 8.5, 9.5))
            if cards:
                ev = display.compute_event(conn, elo, name_a, name_b, "Yellow cards", (2.5, 3.5, 4.5))
...
def main():
    ...
    verbosity.add_argument("--full", action="store_true", help="nível 3: dupla chance, draw no bet, handicap asiático")
    verbosity.add_argument(
        "--json",
        action="store_true",
    ...
    if args.fixtures:
        ...
        for date, h, a, n in rows:
            ...
            data = show(
                ...
                level=level,
                as_json=args.json,
                quiet=args.resumo and not args.json,
