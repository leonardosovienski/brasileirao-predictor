...
SAMPLE = "| Claim ID | Claim | State |\n|---|---|---|\n| CLAIM-BR-TEST-001 | Fictional claim | UNKNOWN |\n"


class ExportTests(unittest.TestCase):
    def setUp(self):
        area = Path("C:/BRASILEIRAO/work/bundle-export-tests")
    ...
    def run_export(self, destination="out", expected=None):
        return export(
            self.root, self.expected if expected is None else expected, self.base / destination, "2026-09-11T00:00:00Z"
        )

    def test_replay_retains_clocks_and_does_not_certify_pit(self):
        import json

        from export_cain_bundle import REPLAY

        path = self.root / REPLAY
        ...
        record = dict(
            ...
            games=[
                dict(
                    event_id="TEST-001",
                    kickoff_at="2026-09-01T00:00:00Z",
        ...
        self.commit()
        self.expected[REPLAY] = digest(path.read_bytes())
        bundle = self.run_export()
        match = next(e for e in bundle["entities"] if e["entity_type"] == "match")
        assert match["event_at"] == record["games"][0]["kickoff_at"]
        assert match["recorded_at"] == record["generated_at"]
        assert match["available_at"] is None
        assert match["payload"]["captured_at"] == record["games"][0]["captured_at"]
        assert match["payload"]["observation_selection"] == "NOT_PROVEN"
        assert not any(e["entity_type"] == "prediction" for e in bundle["entities"])
