...
O mandato BR-CAIN-MAIN-20260912 autoriza também testar o código com fontes versionadas em checkout isolado, sem escrever em dados operacionais ou rodar coortes reais. A CI principal mantém todos os seus critérios; não use `[skip ci]` para contorná-la na consolidação. O workflow delimitado roda também em `main` e não substitui o global. O gate de poder usa os controles sintéticos existentes e seu recibo anterior permanece arquivado. Não inferir homologação econômica de nenhum destes workflows.
