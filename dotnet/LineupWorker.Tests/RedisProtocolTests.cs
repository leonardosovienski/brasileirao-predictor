...
public sealed class RedisProtocolTests
{
    private const string Valid = """
        {"protocol_version":"brasileirao.redis/2","job_id":"j","run_id":"r","match_id":"m","state_version":"1","idempotency_key":"idem","1":2.0,"X":3.0,"2":4.0,"o25":1.9,"u25":2.1}
        """;
    ...
    public void InvocationSerializesSharedFieldNames()
    {
        var payload = new KernelInvokePayload(
            RedisProtocol.Version, "job", "run", "idem", "match", 1600, 1500, 0, 0, 1, "1");
        var json = JsonSerializer.Serialize(payload);
        Assert.Contains("\"protocol_version\":\"brasileirao.redis/2\"", json);
        Assert.Contains("\"idempotency_key\":\"idem\"", json);
        Assert.Contains("\"state_version\":\"1\"", json);
    }
    ...
    [InlineData("true")]
    [InlineData("null")]
    public void RejectsNoncanonicalStateVersions(string invalid)
    {
        using var doc = JsonDocument.Parse(Valid.Replace("\"state_version\":\"1\"", "\"state_version\":" + invalid));
        Assert.Throws<JsonException>(() => FairOddsPayload.FromDict(doc.RootElement));
    }

    [Fact]
    public void RejectsLegacyResponseEvenWhenItContainsNewFields()
    {
        using var doc = JsonDocument.Parse(Valid.Replace("brasileirao.redis/2", "brasileirao.redis/1"));
        Assert.Throws<JsonException>(() => FairOddsPayload.FromDict(doc.RootElement));
    }
}
