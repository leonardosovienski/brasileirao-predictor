...
public sealed partial class WorkerRuntimeTests
{
    ...
    {
        var match = Guid.NewGuid().ToString("N");
        var state = State(match) with { WatchdogDeadlineUnixMs = DateTimeOffset.UtcNow.AddSeconds(-1).ToUnixTimeMilliseconds() };
        var engine = Engine();
        var first = await Register(engine, state);
        var db = _redis.GetDatabase();
        ...
        RedisKey[] keys = ["lineup_state:" + match, KernelRedisProtocolV2.CurrentPrefix + match,
            "fair_odds:" + match, KernelRedisProtocolV2.PendingKey, KernelRedisProtocolV2.ReadyKey, KernelRedisProtocolV2.WatchdogKey];
        ...
        Assert.True((await db.SortedSetScoreAsync(KernelRedisProtocolV2.WatchdogKey, match)).HasValue);
    ...
    public async Task QueueRetriesSameEventBeforeRegistrationAndAfterUncertainCommittedReply()
    {
        ...
        foreach (var failure in new[] { "before", "after", "exhausted", "permanent" })
        {
            ...
            var connection = ProxyDatabase((method, args) =>
            {
                if (method.Name != "ScriptEvaluateAsync" || args![0]!.ToString() != KernelRedisProtocolV2.Register)
