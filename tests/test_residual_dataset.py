...
def test_materializes_only_complete_books_available_at_horizon():
    ...
    assert rows[0]["best_odds_by_selection"] == {"over": 2.1, "under": 1.9}
...
def test_rejects_synthetic_pair_from_different_market_states():
    observations = [
        _quote("a", "over", 2.0, "2026-08-09T10:00:00+00:00"),
        _quote("a", "under", 1.9, "2026-08-09T10:05:00+00:00"),
    ]
    results = [{"source_event_id": "e", "home_goals": 2, "away_goals": 1, "settled_at": "2026-08-11T14:00:00+00:00"}]
    assert materialize_total_market_records(observations, results, horizon_hours=24) == []
