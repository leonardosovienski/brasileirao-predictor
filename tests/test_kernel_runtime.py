...
class _DaemonRedis:
    ...
    async def eval(self, script, count, *args):
        assert script == protocol.HEARTBEAT_SCRIPT and count == 1
        key, value, ttl, mode = args
        assert key == protocol.HEALTH_KEY and ttl == 5000
        if mode == "start":
            self.set_calls.append((key, json.loads(value)["protocol_version"]))
        elif mode == "release":
            self.deleted.append(key)
        return 1
...
def test_daemon_sets_health_and_cleans_up_on_shutdown(monkeypatch) -> None:
    ...
    client = _DaemonRedis()
    monkeypatch.setattr(redis_asyncio_module, "from_url", lambda url, **kwargs: client)
    ...
    async def exercise() -> None:
        ...
        loop = SimpleNamespace(
            ...
            remove_signal_handler=lambda signal: True,
        )
    ...
    assert client.pubsub_instance.subscribed == [protocol.INVOKE_CHANNEL]
    assert client.set_calls == [("health:kernel", protocol.PROTOCOL_VERSION)]
    assert client.pubsub_instance.unsubscribed == [protocol.INVOKE_CHANNEL]
