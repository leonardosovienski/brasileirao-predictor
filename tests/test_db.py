    ...
    db.upsert_ss_matches(
        ...
        [
            _row(
                ...
                odds_home=2.60,
                odds_draw=3.60,
                odds_away=2.80,  # fechamento mexeu
                odds_home_open=None,
                odds_draw_open=None,
                odds_away_open=None,
                odds_over_open=None,
                odds_under_open=None,
            )
        ],
    )

    oh, oh_open = conn.execute("SELECT odds_home, odds_home_open FROM sofascore_matches WHERE event_id=1").fetchone()
...
def test_close_sempre_sobrescreve(conn):
    ...
    ou = conn.execute("SELECT odds_under FROM sofascore_matches WHERE event_id=2").fetchone()[0]
    assert ou == 1.55


def test_upsert_resolve_placeholder_de_bracket_para_nome_real(conn):
    """Bug real (2026-07-05): a 1ª coleta grava o evento com placeholder de
    bracket ('W83'/'W84', antes do Sofascore saber quem classificou); a 2ª
    coleta do MESMO event_id já traz o nome resolvido ('Mexico'/'England').
    Sem home_team/away_team no ON CONFLICT DO UPDATE, o upsert mantinha o
    placeholder da 1ª coleta pra sempre — parecia atraso do Sofascore, era o
    upsert descartando o nome novo em silêncio."""
    db.upsert_ss_matches(conn, [_row(20, home_team="W83", away_team="W84")])
    db.upsert_ss_matches(conn, [_row(20, home_team="Mexico", away_team="England", odds_home=2.05)])
    home, away = conn.execute("SELECT home_team, away_team FROM sofascore_matches WHERE event_id=20").fetchone()
...
def test_snapshots_idempotentes(conn):
    ...
    assert n1 == 1
    assert n2 == 0
    total = conn.execute("SELECT COUNT(*) FROM odds_snapshots WHERE event_id=10").fetchone()[0]
    assert total == 1
...
def test_snapshots_acumulam_no_tempo(conn):
    ...
    n = conn.execute("SELECT COUNT(*) FROM odds_snapshots WHERE event_id=11").fetchone()[0]
    assert n == 2
...
def test_update_ht_scores_grava_e_none_e_noop(conn):
    ...
    ht = conn.execute("SELECT home_score_ht, away_score_ht FROM sofascore_matches WHERE event_id=30").fetchone()
    assert ht == (1, 0)
...
def test_update_kickoff_grava_utc_e_none_e_noop(conn):
    ...
    value = conn.execute("SELECT kickoff_at FROM sofascore_matches WHERE event_id=31").fetchone()[0]
