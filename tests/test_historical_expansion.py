...
from src.data.historical_expansion import (
    canonical_team, connect_shadow, coverage_report, ingest_api_football,
)


def _row(event="1", home="Sao Paulo", away="Flamengo", hg=2, ag=1):
    return {"source": "api_football", "source_event_id": event, "season": 2024,
            "scheduled_at": "2024-05-01T20:00:00+00:00", "home_team": home,
            "away_team": away, "home_goals": hg, "away_goals": ag,
            "status": "FT", "shadow_only": True}
...
def test_report_detects_duplicates_new_rows_and_score_conflicts(tmp_path):
    ...
    ingest_api_football(shadow, [
        _row("1"),
        {**_row("2", "Bahia", "Flamengo", 0, 0),
         "scheduled_at": "2024-05-02T20:00:00+00:00"},
    ], known_teams=known)
