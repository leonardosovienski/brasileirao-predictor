...
def test_p10_history_no_formato_errado_falha_fechado():
    ...
    buggy_history = [
        (
            1500 + rng.integers(-100, 100),
            1450 + rng.integers(-100, 100),
        ...
        hist.append(
            {
                "home_team": "A",
                "away_team": "B",
                "home_elo": 1500 + diff / 2,
                "away_elo": 1500 - diff / 2,
...
def test_p8_lambda_home_diferente_de_away_para_diff_positivo():
    params = fit_event_model(_synthetic_event_history(), "corners", distribution="poisson")
        ...
        hist.append(
            {
                ...
                "home_elo": 1500 + diff / 2,
                "away_elo": 1500 - diff / 2,
                "home_event": int(rng.poisson(lam_h)),
                "away_event": int(rng.poisson(lam_a)),
            }
        )
    return hist
