...
from src.data.pit_backfill import (
    CLOSING_DEFINITION_VERSION, curate_match, choose_closing, connect_curated,
    evaluation_view, pit_eligible, preserve_raw, register_raw, resolve_entity,
    curate_odds,
    quality_gate, walk_forward_splits,
    verify_raw,
)
...
def _match(**extra):
    row = {
        "source": "fixture", "source_match_id": "m1",
        "kickoff_at": "2024-05-01T20:00:00+00:00",
        "home_team": "Sao Paulo", "away_team": "Flamengo",
...
def test_raw_is_write_once_hashable_and_tamper_detected(tmp_path):
    ...
    manifest = preserve_raw(b'{"id":1}', raw, source="fixture",
                            source_version="v1", retrieved_at="2024-01-01T00:00:00Z",
                            license="test", parser_version="p1", row_count=1,
                            temporal_coverage="2024")
    verify_raw(raw, manifest)
    with pytest.raises(FileExistsError):
        preserve_raw(b"changed", raw, source="fixture", source_version="v1",
                     retrieved_at="2024-01-01T00:00:00Z", license="test",
                     parser_version="p1", row_count=1, temporal_coverage="2024")
...
def test_curated_store_has_provenance_and_never_touches_live(tmp_path):
    ...
    manifest = preserve_raw("{}", raw, source="fixture", source_version="v1",
                            retrieved_at="2024-01-01T00:00:00Z", license="test",
                            parser_version="p1", row_count=1, temporal_coverage="2024")
    register_raw(conn, manifest)
    cid = curate_match(conn, _match(), aliases={"Sao Paulo": "São Paulo"},
                       known={"São Paulo", "Flamengo"}, batch_id="b1",
...
def test_closing_requires_valid_pre_kickoff_quote_and_versions_definition():
    rows = [
        {"bookmaker": "A", "market": "ou2.5", "selection": "over",
         "raw_odds": 2.0, "captured_at": "2024-04-30T20:00:00Z"},
        {"bookmaker": "A", "market": "ou2.5", "selection": "over",
         "raw_odds": 2.1, "captured_at": "2024-05-01T19:00:00Z"},
        {"bookmaker": "A", "market": "ou2.5", "selection": "over",
         "raw_odds": 9.0, "captured_at": "2024-05-01T21:00:00Z"},
    ]
    close = choose_closing(rows, kickoff_at="2024-05-01T20:00:00Z",
                           bookmaker="A", market="ou2.5", selection="over")
...
def test_curated_odds_rejects_future_capture_and_preserves_all_clocks(tmp_path):
    ...
    row = {
        ...
        "captured_at": "2024-05-01T10:02:00Z", "bookmaker": "A",
        "market": "ou2.5", "selection": "over", "raw_odds": 2.0,
    }
    ...
    with pytest.raises(ValueError, match="pré-evento"):
        curate_odds(conn, {**row, "source_match_id": "m2",
                           "captured_at": "2024-05-01T21:00:00Z"},
                    canonical_match_id="cid", batch_id="b1")


def test_pit_rejects_future_and_evaluation_view_is_temporal(tmp_path):
    assert pit_eligible(available_at="2024-05-01T10:00:00Z",
                        predicted_at="2024-05-01T11:00:00Z",
                        kickoff_at="2024-05-01T20:00:00Z")
    assert not pit_eligible(available_at="2024-05-01T21:00:00Z",
                            predicted_at="2024-05-01T11:00:00Z",
                            kickoff_at="2024-05-01T20:00:00Z")
    conn = connect_curated(tmp_path / "eval.db")
    curate_match(conn, _match(source_match_id="past"), aliases={"Sao Paulo": "São Paulo"},
                 known={"São Paulo", "Flamengo"}, batch_id="b1",
                 ingested_at="2024-04-01T00:00:00Z")
    curate_match(conn, _match(source_match_id="future", kickoff_at="2024-06-01T20:00:00Z"),
                 aliases={"Sao Paulo": "São Paulo"}, known={"São Paulo", "Flamengo"},
                 batch_id="b1", ingested_at="2024-04-02T00:00:00Z")
    assert [r["source_match_id"] for r in evaluation_view(
        conn, predicted_at="2024-05-15T00:00:00Z")] == ["future"]


def test_walk_forward_holdout_and_cluster_gate_never_enable_capital():
    rows = [{"kickoff_at": f"2024-0{month}-15T12:00:00Z", "id": month}
            for month in (1, 2, 3, 4)]
    split = walk_forward_splits(rows, development_end="2024-02-01T00:00:00Z",
    ...
    observations = [{"pit_valid": True, "matured": True, "cluster": "r1",
                     "clubs": ("A", "B"), "pnl": 1.0, "clv": .1, "brier": .2},
                    {"pit_valid": True, "matured": False, "cluster": "r1",
                     "clubs": ("A", "B"), "pnl": 1.0, "clv": .1, "brier": .2}]
