...
from brasileirao_scripts import exp001_data_pilot
...
@pytest.mark.parametrize("error_class", [requests.ConnectionError, requests.Timeout])
def test_transport_exception_never_exposes_key_or_url_in_traceback(monkeypatch, error_class):
    key = "SYNTHETIC_SECRET_NOT_A_CREDENTIAL"

    def unavailable(*args, **kwargs):
        raise error_class(f"https://example.invalid/?apiKey={key}")

    monkeypatch.setattr(exp001_data_pilot.requests, "get", unavailable)
    with pytest.raises(RuntimeError, match="provider transport request failed") as caught:
        exp001_data_pilot._get("historical-odds", key)
