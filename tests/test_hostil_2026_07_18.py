...
spec = importlib.util.spec_from_file_location(
    "sombra_hostil", ROOT / "scripts" / "sombra.py")
...
def _cfg():
    return {"backtest": {"min_edge": 0.02, "max_edge": 0.15,
                         "over_under_line": 2.5},
            "model": {"max_goals": 12}, "elo": {"home_advantage": 100}}
...
def _pick(**kw):
    base = {"captured_at": "2026-07-10T00:00:00+00:00", "event_id": 10,
            "date": "2026-07-16", "home": "Casa", "away": "Fora",
            "market": "ou2.5", "selection": "under", "odd": 1.95,
            "edge": 0.03, "model_prob": 0.55, "trial": "h3-ou25-sombra-2026"}
    base.update(kw)
    return base
...
@pytest.mark.parametrize("c_over,c_under", [
...
def test_capture_recusa_linha_inteira(tmp_path):
    ...
    cfg["backtest"]["over_under_line"] = 3.0
    with pytest.raises(SystemExit, match="linha INTEIRA"):
        sombra._capture_funil(cfg, conn, lambda h, a: None,
                              tmp_path / "p.jsonl", "t")
...
def test_record_result_rejeita_placar_negativo(tmp_path):
    with pytest.raises(ValueError, match="negativo"):
        record_result("Casa", "Fora", -1, 2,
                      path=tmp_path / "res.jsonl", pred_path=tmp_path / "none.jsonl")
