"""evaluate_h15_prospective: gate de poder mecânico e recusa a reescrever
avaliação já feita — mesma estrutura de test_evaluate_h14_prospective.py."""

import json
from pathlib import Path

from brasileirao_predictor import db
from brasileirao_scripts import evaluate_h15_prospective as job
...
def _seed_db_and_ledger(tmp_path: Path, n: int) -> tuple[Path, Path]:
    ...
    for i in range(n):
        ...
        record = {
            ...
            "home": "A",
            "away": "B",
            "treatment_refit10": {"p_home": 0.6, "p_draw": 0.2, "p_away": 0.2},
            "control_refit100": {"p_home": 0.5, "p_draw": 0.25, "p_away": 0.25},
        }
...
def test_abaixo_do_n_minimo_nao_calcula_nada(tmp_path):
    ...
    result = job.evaluate(
        trials_path=trials_path, ledger_path=ledger_path, db_path=dbpath, reports_dir=tmp_path / "reports"
    )
...
def test_main_recusa_sobrescrever_relatorio_existente(tmp_path, monkeypatch):
    ...
    assert job.main() == 0
    assert len(list(reports_dir.glob("h15_avaliacao_*.json"))) == 1
            ...
            json.dumps(
                {
                    ...
                    "home": "C",
                    "away": "D",
                    "treatment_refit10": {"p_home": 0.5, "p_draw": 0.3, "p_away": 0.2},
                    "control_refit100": {"p_home": 0.4, "p_draw": 0.3, "p_away": 0.3},
                }
            )
            + "\n"
        )
