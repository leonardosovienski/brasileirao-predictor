...
def test_migration_adds_flat_columns_to_old_db():
    ...
    conn.execute("INSERT INTO sofascore_matches (event_id, odds_over, odds_under) VALUES (1, 1.9, 1.95)")
    ...
    # dado antigo intacto
    assert conn.execute("SELECT odds_over FROM sofascore_matches WHERE event_id=1").fetchone()[0] == 1.9
...
def test_backfill_ou_2_5_into_odds_lines():
    conn = _fresh()
    conn.execute(
        "INSERT INTO sofascore_matches (event_id, odds_over, odds_under, "
        "odds_over_open, odds_under_open) VALUES (7, 2.1, 1.8, 2.2, 1.75)"
    )
    db._migrate(conn)  # backfill roda aqui
    row = conn.execute(
        "SELECT odd_a, odd_b, odd_a_open, odd_b_open FROM odds_lines WHERE event_id=7 AND market='ou' AND line=2.5"
    ).fetchone()
...
def _insert_event(conn, eid=100):
    conn.execute(
        "INSERT INTO sofascore_matches (event_id, home_team, away_team) VALUES (?, 'A', 'B')",
        (eid,),
    )
    conn.commit()
...
def test_update_flat_markets_close_and_open():
    ...
    _insert_event(conn)
    close = {
        "dc": {"1X": 1.2, "X2": 1.9, "12": 1.4},
        "dnb": {"1": 1.25, "2": 3.75},
        "btts": {"Yes": 2.0, "No": 1.7},
    }
    open_ = {
        "dc": {"1X": 1.25, "X2": 1.85, "12": 1.45},
        "dnb": {"1": 1.30, "2": 3.50},
        "btts": {"Yes": 2.1, "No": 1.65},
    }
    db.update_flat_markets(conn, 100, close, open_)
    r = conn.execute(
        "SELECT odds_dc_1x, odds_dnb_home, odds_btts_yes, "
...
def test_flat_open_is_write_once():
    ...
    db.update_flat_markets(conn, 100, {"btts": {"Yes": 2.0, "No": 1.7}}, {"btts": {"Yes": 2.0, "No": 1.7}})
    ...
    r = conn.execute("SELECT odds_btts_yes, odds_btts_yes_open FROM sofascore_matches WHERE event_id=100").fetchone()
...
def test_lines_rows_from_parsed_pure():
    close = {
        "ou": {1.5: {"Over": 1.4, "Under": 3.0}, 2.5: {"Over": 2.0, "Under": 1.8}},
        "ah": {-0.75: {"home": 2.0, "away": 1.8}},
    }
    open_ = {
        "ou": {1.5: {"Over": 1.45, "Under": 2.9}, 2.5: {"Over": 2.05, "Under": 1.75}},
...
def test_upsert_odds_lines_close_overwrites_open_write_once():
    ...
    db.upsert_odds_lines(
        conn,
        db.lines_rows_from_parsed(
            5,
    ...
    db.upsert_odds_lines(conn, db.lines_rows_from_parsed(5, {"ou": {2.5: {"Over": 1.7, "Under": 2.1}}}, {}))
    row = conn.execute(
        "SELECT odd_a, odd_b, odd_a_open, odd_b_open FROM odds_lines WHERE event_id=5 AND market='ou' AND line=2.5"
    ).fetchone()
