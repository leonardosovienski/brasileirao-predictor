...
def _fixture(conn):
    conn.execute(
        ...
        "(event_id, competition, season, date, kickoff_at, home_team, away_team, home_score, away_score) "
        "VALUES (7, 'Brasileirão', '2026', '2026-07-20', "
        "'2026-07-20T20:00:00+00:00', 'Casa', 'Fora', 2, 1)"
    )
    conn.commit()
...
def test_collection_only_is_idempotent_and_never_uses_shadow_state(tmp_path: Path):
    ...
    first = collect(conn, root=Path.cwd(), archive_path=archive, observed_at=datetime(2026, 7, 21, tzinfo=UTC))
    second = collect(conn, root=Path.cwd(), archive_path=archive, observed_at=datetime(2026, 7, 21, tzinfo=UTC))
...
def test_retry_resumes_event_started_without_lifecycle_regression(tmp_path: Path):
    ...
    conn.execute(
        "INSERT INTO sofascore_matches "
        "(event_id, competition, season, date, kickoff_at, home_team, away_team) "
        "VALUES (8, 'Brasileirão', '2026', '2026-07-20', "
        "'2026-07-20T20:00:00+00:00', 'Casa', 'Fora')"
    )
    conn.commit()
    archive = tmp_path / "archive.jsonl"
