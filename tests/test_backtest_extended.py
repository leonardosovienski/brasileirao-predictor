...
def _ss_row(event_id, home, away, date):
    ...
    base = dict(
        ...
        odds_away_open=2.40,
        odds_over_open=1.85,
        odds_under_open=2.00,
    )
    return tuple(base.values())


def _setup(conn, home="Brazil", away="Serbia", hs=2, as_=0, eid=1, dc=None, btts=None, ou_lines=None):
    conn.execute(
        "INSERT INTO matches (date, home_team, away_team, home_score, away_score,"
        " tournament, city, country, neutral) "
        "VALUES ('2026-06-20',?,?,?,?,'Friendly','','',0)",
        (home, away, hs, as_),
    )
    conn.commit()
...
def test_double_chance_swap_orientation(conn):
    ...
    _setup(
        ...
        away="Brazil",
        hs=0,
        as_=2,  # placar na orient. sofascore
        dc={"1X": 9.99, "X2": 1.23, "12": 1.4},
    )
    # martj42: Brazil venceu (away no banco sofascore, home no matches)
    conn.execute(
        "UPDATE matches SET home_team='Brazil', away_team='Serbia', home_score=2, away_score=0 WHERE home_team='Serbia'"
    )
    conn.commit()
    _params, ledger = run_backtest(CFG, conn)
    dc = {b["selection"]: b for b in ledger if b["market"] == "dc"}
