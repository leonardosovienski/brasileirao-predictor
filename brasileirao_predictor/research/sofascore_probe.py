...
USO:
    ...
    python -m brasileirao_predictor.research.sofascore_probe

    # ou aponta um event_id específico
    python -m brasileirao_predictor.research.sofascore_probe --event-id 14025190
"""
...
def _pick_event(conn) -> int | None:
    ...
    row = conn.execute(
        "SELECT event_id FROM sofascore_matches WHERE home_score IS NULL ORDER BY date LIMIT 1"
    ).fetchone()
    if row:
        return row[0]
    row = conn.execute("SELECT event_id FROM sofascore_matches ORDER BY date DESC LIMIT 1").fetchone()
...
def run(event_id: int | None = None) -> int:
    ...
    if event_id is None:
        ...
        if event_id is None:
            print("Nenhum event_id em sofascore_matches — rode a coleta primeiro.", file=sys.stderr)
            return 1
