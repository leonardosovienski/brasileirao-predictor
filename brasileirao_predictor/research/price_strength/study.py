...
def _quote_received_at(row: dict) -> datetime | None:
    """An unreadable receipt is unknown, never proof of future availability.

    Full snapshot validation remains with scan_quotes, which retains the error
    and blocks a batch when an invalid receipt prevents ordering its state.
    """
    try:
        return timestamp(row.get("received_at"))
    except (TypeError, ValueError):
        return None


def _validate_quote_event_mapping(quotes: list[dict], decisions: dict[str, datetime]) -> None:
    # Per-fixture scans cannot see a provider event split across two fixtures.
    # Check the study-wide mapping only from receipts visible for their own
    # declared fixture decision; future remappings must not rewrite that past.
    mapped: dict[tuple[str, str], str] = {}
    for row in quotes:
        event_id = row["event_id"]
        received = _quote_received_at(row)
        if received is None or received > decisions[event_id]:
            continue
        source, source_event_id = row.get("source"), row.get("source_event_id")
        if not isinstance(source, str) or not isinstance(source_event_id, str):
            continue  # Invalid source fields remain subject to snapshot validation.
        identity = (source, source_event_id)
        if any(not value.strip() or value != value.strip() for value in identity):
            continue  # Invalid source fields remain subject to snapshot validation.
        previous = mapped.setdefault(identity, event_id)
        if previous != event_id:
            raise ValueError("source event maps to multiple canonical fixture IDs")
...
def run_study(
    *, protocol: dict, history: list[dict], fixtures: list[dict], quotes: list[dict], baseline: list[dict] | None = None
    ...
    _validate_quote_event_mapping(quotes, {fixture.match_id: decision for decision, fixture in planned})
    baseline_index = {}
    ...
    for decision, fixture in planned:
        ...
        for quote in event_quotes:
            received = _quote_received_at(quote)
            if received is None or received > decision:
                continue  # Unknown clocks are validated by scan_quotes, never treated as future.
