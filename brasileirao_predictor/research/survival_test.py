...
Uso:
    python -m brasileirao_predictor.research.survival_test \\
...
from brasileirao_predictor.research.score_metrics import (
...
def _predict_hybrid(elo_a, elo_b, params, dvorp_a, dvorp_b, theta) -> np.ndarray:
    ...
    res = predict_match(
        ...
        params,
        home_adv=0.0,
        delta_vorp_a=dvorp_a,
        delta_vorp_b=dvorp_b,
        theta=theta,
    )
    return res["grid"].astype(np.float32)
...
def _team_delta_vorp(team, event_id, presence, beta_players, replacement_levels, positions):
    ...
    players = [p for p, (t, _) in presence.get(event_id, {}).items() if t == team]
    if not players:
        return 0.0
...
def run(db_path: str, vorp_path: str | None, theta: float, debug_team: str | None, kelly_frac: float):
    ...
    pos_rows = conn.execute("SELECT player, position FROM player_comp_stats WHERE position IS NOT NULL").fetchall()
    pos_count = {}
    ...
    if not test_rows:
        sys.exit(
            f"Nenhuma partida encerrada em seasons {TEST_SEASONS}. "
    ...
    if use_vorp:
        ...
        pres_rows = conn.execute(
            ...
            f"WHERE event_id IN ({ph}) AND minutes > 0",
            list(all_eids),
        ).fetchall()
    ...
    for eid, date, home, away, hs, as_, hxg, axg, oh, od, oa, _ in test_rows:
        ...
        dvorp_a = dvorp_b = 0.0
        if use_vorp:
            dvorp_a = _team_delta_vorp(home, eid, presence, beta_players, replacement_levels, positions)
            dvorp_b = _team_delta_vorp(away, eid, presence, beta_players, replacement_levels, positions)
    ...
    print(f"  p-valor : {dm_pval:.4f}  " + ("*** SIGNIFICATIVO (p<0.05)" if dm_pval < 0.05 else "(não significativo)"))
    ...
    if len(pnl) >= 3:
        ...
        print(f"  PSR (P(SR>0))     : {psr:.4f}  " + ("*** POSITIVO" if psr > 0.9 else "(insuficiente)"))
    else:
...
def main():
    ...
    parser.add_argument("--debug-team", default=None, help="time para debug isolado (ex: 'Athletico Paranaense')")
    parser.add_argument("--kelly-frac", type=float, default=0.25, help="fração de Kelly para simulação de P&L")
