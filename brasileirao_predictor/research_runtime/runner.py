"""`brasileirao-research`: composition root of the Brasileirão research circuit (file requests).

    request file -> admission (AdmissionStore, operator policy) -> reference materialization
    (ReferenceStore: JSON references + hashed SQLite snapshot) -> handler from the compiled
    allowlist run as a real predictor_ops job (jobs file v3 + python -m predictor_ops run)
    -> predictor_core replay/metrics/bootstrap/trial registry -> ResultStore (authoritative)
...
Commands (all take --state DIR; process/run also --policy FILE --objects DIR):
    ...
    put-object FILE          operator provisioning of an immutable JSON reference object
    put-dataset --source DB --as-of T --label L   operator snapshot of a SQLite database

The data root is never implicit: dataset, model, features, baseline, cost model and odds come
only from the operator object store (--objects), selected by name/version in the policy
registry and verified by sha256. No checkout, .git or project root is read.

Each submission writes one outcome file to <state>/outcomes/ and prints it as one JSON line.
Exit code: the highest per-request code in contract.EXIT_CODES (0 result/duplicate,
2 rejected/conflict, 3 not ready/retryable Ops failure, 4 temporal integrity violation,
5 reconciliation required); 1 = operator configuration error.
"""
...
from brasileirao_predictor.research_runtime.durable import atomic_write
from brasileirao_predictor.research_runtime.execution import (
    EXEC_DIR,
    EXPERIMENTS_DIR,
...
from brasileirao_predictor.research_runtime.results import ResultConflict, ResultIntegrityError, ResultStore
...
class Circuit:
    """Real composition of the Brasileirão research components for one state root."""
    ...
    def _write_outcome(self, outcome: dict) -> dict:
        ...
        return outcome | {"outcome_file": str(path)}
    ...
    def submit_request(self, raw: bytes, *, source: str) -> dict:
        ...
        base = {"schema": OUTCOME_SCHEMA, "submission_file": source, "submission_sha256": digest(raw), "at": _now()}
        try:
            ...
            try:
                ...
                pass
            return self._finish(
                base | {"request_id": request_id, "status": "REJECTED", "reason": exc.reason, "detail": str(exc)[:300]}
            )
        ...
        try:
            ...
            return self._finish(
                base | {"status": "RECONCILIATION_REQUIRED", "reason": type(exc).__name__, "detail": str(exc)[:300]}
            )
    ...
    def show(self, request_id: str) -> tuple[int, dict]:
        try:
            ...
            return 5, {"status": "RECONCILIATION_REQUIRED", "request_id": request_id, "reason": str(exc)[:300]}
        if result is None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="brasileirao-research", description=(__doc__ or "").splitlines()[0])
    ...
    dataset = sub.add_parser("put-dataset")
    dataset.add_argument("--objects", type=Path, required=True)
    dataset.add_argument("--source", type=Path, required=True)
    dataset.add_argument("--as-of", required=True)
    dataset.add_argument("--label", required=True)
    ...
    if args.command == "put-object":
        ...
        return 0
    if args.command == "put-dataset":
        try:
            stored = ReferenceStore(args.objects).put_dataset(args.source, as_of=args.as_of, label=args.label)
        except (ContractError, ValueError, OSError) as exc:
            print(json.dumps({"status": "CONFIGURATION_ERROR", "error": str(exc)[:300]}), file=sys.stderr)
            return 1
        print(json.dumps(stored, sort_keys=True, ensure_ascii=False))
        return 0
    if args.command == "restore":
        from brasileirao_predictor.research_runtime.recovery import restore_recovery_bundle
    ...
    if args.command == "backup":
        from brasileirao_predictor.research_runtime.recovery import create_recovery_bundle
        ...
        manifest = create_recovery_bundle(args.out, sqlite_databases=databases, artifact_roots=roots)
        print(json.dumps({"bundle": str(args.out), "files": len(manifest["files"])}, sort_keys=True))
        return 0
    ...
    try:
        circuit = Circuit(args.state, args.policy, args.objects)
