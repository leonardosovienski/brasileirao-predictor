"""Brasileirão-owned authoritative result store (no envelope, no signing, no transport).
...
from brasileirao_predictor.research_runtime.contract import canonical, content_hash, validate_result
        ...
        with self.connection() as db:
            ...
            for row in rows:
                if (
                    ...
                    raise ResultConflict("CONFLICT: a different result already exists for this identity")
            if rows:
