...
def build_features(db_path: str, event_id: int, window: int = 10) -> dict:
    ...
    row = conn.execute(
        "SELECT date, home_team, away_team FROM sofascore_matches WHERE event_id = ?", (event_id,)
    ).fetchone()
...
def _team_features(conn: sqlite3.Connection, team: str, match_date: str, window: int) -> dict:
    """
    ...
    event_rows = conn.execute(
        ...
        LIMIT ?
    """,
        (team, team, team, match_date, window),
    ).fetchall()
