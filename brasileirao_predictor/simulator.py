...
def monte_carlo(n=10000, seed=None, vorp=None):
    ...
    if not elo or not prow:
        sys.exit("cache vazio — rode `python -m brasileirao_predictor.cron_update_models` primeiro")
    params = (prow[0], prow[1], prow[2], prow[3])
    # NOTA Brasileirão: o simulador é de TORNEIO com fase de grupos (herança da
    # Copa) — liga de pontos corridos não tem grupos de 4; aqui ele encerra com
