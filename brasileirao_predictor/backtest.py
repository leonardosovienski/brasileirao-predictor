...
de apostas, não milhares. Significância: `python -m brasileirao_predictor.bootstrap`.
"""
...
def _report(ledger):
    ...
    print(
        ...
        "\n      por construção. Sinal real só na população 'open'. IC 95%: brasileirao_predictor.bootstrap"
    )
...
def main():
    ...
    if not out:
        sys.exit("sem odds históricas no banco — rode `python -m brasileirao_predictor.ingest_sofascore`")
    params, ledger = out
    if not ledger:
