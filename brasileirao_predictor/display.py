...
def get_clv_summary():
    """Lê data/bootstrap_cache.json (gravado por `python -m brasileirao_predictor.bootstrap`).
...
def _clv_line(cache, market):
    ...
    if not cache or market not in cache.get("markets", {}):
        return f"CLV ({label}): sem dado ainda (rode `python -m brasileirao_predictor.bootstrap`)"
