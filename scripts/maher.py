...
a_b, b_b, alpha, rho = model.fit_goal_model(
    [h for h, r in zip(hist, rows) if ccut <= r[0] < ftd]
)  # baseline + alpha/rho compartilhados
K = np.arange(MAXG + 1)
...
def fit_maher(win, lam_reg):
    teams = sorted(set([g[1] for g in win]) | set([g[2] for g in win]))
...
wtrain = [r for r in window(tv)]
wval = [r for r in rows if tv <= r[0] < t0]
best = (None, 1e18)
for lr in (1, 3, 10, 30, 100):
    par = fit_maher(wtrain, lr)
    dev = 0.0
    for g in wval:
        lh, la = maher_lambda(par, g)
        dev += (lh - g[3] * math.log(max(lh, 1e-9))) + (la - g[4] * math.log(max(la, 1e-9)))
    if dev < best[1]:
        best = (lr, dev)
LR = best[0]
print(f"lambda_reg escolhido (val 6m): {LR}\n")
...
for y, m in months:
    ...
    for i, r in enumerate(rows):
        ...
        lh, la = maher_lambda(par, r)
        pM = grid_probs(lh, la)
        d = hist[i][0] / 400.0
        pB = grid_probs(math.exp(a_b + b_b * d), math.exp(a_b - b_b * d))
        mb.append(brier(pM, y3))
        mb_b.append(brier(pB, y3))
        ll.append(-math.log(max(pM[y3], 1e-9)))
        ll_b.append(-math.log(max(pB[y3], 1e-9)))
        mx.append(max(pM))
        mx_b.append(max(pB))
        pd.append(pM[1])
        pd_b.append(pB[1])
        obs.append(1 if y3 == 1 else 0)

print(f"holdout: {len(mb)} jogos | {len(months)} refits mensais\n")
print(f"{'modelo':<14}{'Brier':>9}{'LogL':>9}{'probMax':>9}{'P(draw)':>9}{'gap_draw':>9}")
print(
    f"{'BASELINE Elo':<14}{st.mean(mb_b):>9.4f}{st.mean(ll_b):>9.4f}{st.mean(mx_b):>9.1%}"
    f"{st.mean(pd_b):>9.1%}{st.mean(pd_b) - st.mean(obs):>+9.1%}"
)
print(
    f"{'MAHER atk/def':<14}{st.mean(mb):>9.4f}{st.mean(ll):>9.4f}{st.mean(mx):>9.1%}"
    f"{st.mean(pd):>9.1%}{st.mean(pd) - st.mean(obs):>+9.1%}"
)
d = st.mean(mb_b) - st.mean(mb)
verdict = "MELHOR" if d > 0 else "PIOR/IGUAL"
print(f"\nDelta Brier (Maher - baseline): {st.mean(mb) - st.mean(mb_b):+.4f}  -> Maher {verdict}")
