...
PASSOS = [
    (
        ...
        [
            ...
            "-X",
            "utf8",
            str(ROOT / "scripts" / "record_odds_smoke.py"),
            "--region",
            "eu",
        ],
        120,
    ...
    (
        "odds_stability",
        [sys.executable, "-X", "utf8", str(ROOT / "scripts" / "record_odds_smoke.py"), "--report"],
        60,
    ),
    ...
    (
        "collection_only",
        [sys.executable, "-X", "utf8", str(ROOT / "scripts" / "collect_collection_only.py")],
        300,
    ),
    ("cron_models", [sys.executable, "-X", "utf8", "-m", "src.cron_update_models"], 600),
    (
        "settle",
        [sys.executable, "-X", "utf8", str(ROOT / "scripts" / "sombra.py"), "--settle"],
        300,
    ),
    (
        "capture",
        [sys.executable, "-X", "utf8", str(ROOT / "scripts" / "sombra.py"), "--capture"],
        300,
    ),
    (
        "report",
        [sys.executable, "-X", "utf8", str(ROOT / "scripts" / "sombra.py"), "--report"],
        120,
    ),
]
