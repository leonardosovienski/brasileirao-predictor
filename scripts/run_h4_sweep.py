...
from predictor_core.contracts.registry import (                         # noqa: E402
...
def load_observations(cutoff: str) -> list[dict[str, Any]]:
    ...
    try:
        rows = conn.execute(
            ...
            "AND date <= ? ORDER BY date", (cutoff,)).fetchall()
    finally:
        conn.close()
    obs: list[dict[str, Any]] = []
    for d, home, away, hs, asc in rows:
        kickoff = datetime.fromisoformat(d).replace(tzinfo=timezone.utc)
        obs.append({"home": home, "away": away, "kickoff": kickoff,
...
def sweep_one(half_life: int, observations: list[dict[str, Any]],
              *, min_history: int, retrain_every: int) -> dict[str, Any]:
    """Walk-forward de uma janela: retorna RPS médio + série por-jogo."""
    t0 = time.perf_counter()
    ev = BrasileiraoDixonColesEvaluator(half_life_days=float(half_life),
                                        max_goals=MAX_GOALS)
    log.info("half_life=%3dd | xi=%.5f | walk-forward sobre %d jogos "
             "(min_history=%d, retrain_every=%d)...",
             half_life, ev.xi, len(observations), min_history, retrain_every)
    results = ev.run(observations, min_history=min_history,
                     retrain_every=retrain_every)
    probs = [[r["prediction"].value["home"], r["prediction"].value["draw"],
              r["prediction"].value["away"]] for r in results]
    outcomes = [outcome_index(r["actual"]) for r in results]
    mean_rps = rps(probs, outcomes)
    elapsed = time.perf_counter() - t0
    log.info("half_life=%3dd | RPS=%.5f | %d previsões | rho=%.4f gamma=%.3f | %.1fs",
             half_life, mean_rps, len(results),
             ev.fitted_parameters["rho"], ev.fitted_parameters["home_advantage"],
             elapsed)
    return {"half_life": half_life, "rps": mean_rps, "n_predictions": len(results),
            "probs": probs, "outcomes": outcomes, "elapsed_s": elapsed,
...
def _rps_pipeline(series: tuple[list[list[float]], list[int]]) -> dict[str, str]:
    ...
    lo, _, _ = bootstrap_ci(gains, lambda u: sum(u) / len(u),
                            scheme="iid", n_boot=500, seed=13)
...
def attest_rps_power() -> dict[str, Any]:
    ...
    edge = lambda: probabilistic_predictor(300, skill_level=0.6, seed=13, n_classes=3)
    noise = lambda: probabilistic_predictor(300, skill_level=0.0, seed=17, n_classes=3)
    return attest_pipeline_power(
        _rps_pipeline, edge, noise,
        attestation_path=attestation_path_for(TRIALS),
        note="H4: controle positivo da régua RPS (edge sintético k=3 detectado, "
             "ruído uniforme rejeitado)",
        metric="rps")
...
def main() -> int:
    ...
    parser.add_argument("--quick", action="store_true",
                        help="grade reduzida + retrain esparso (smoke da esteira)")
    parser.add_argument("--cutoff", default=date.today().isoformat(),
                        help="data máxima dos jogos (default: hoje)")
    ...
    log.info("base: %d jogos encerrados até %s | grade: %s", len(observations),
             args.cutoff, grid)
    if len(observations) < MIN_HISTORY + 50:
        log.error("histórico insuficiente (%d) para min_history=%d",
                  len(observations), MIN_HISTORY)
        return 1

    t0 = time.perf_counter()
    sweep = [sweep_one(hl, observations, min_history=MIN_HISTORY,
                       retrain_every=retrain) for hl in grid]
    total = time.perf_counter() - t0

    sweep.sort(key=lambda s: s["rps"])
    log.info("---- resultado da varredura (%.1f min) ----", total / 60)
    for s in sweep:
        log.info("  half_life=%3dd  RPS=%.5f  (n=%d, %.0fs)",
                 s["half_life"], s["rps"], s["n_predictions"], s["elapsed_s"])
    winner = sweep[0]
    ...
    registry.register(
        TRIAL_NAME,
        params={"model": "dixon_coles_wnll", "half_life_days": winner["half_life"],
                "min_history": MIN_HISTORY, "retrain_every": retrain,
                "max_goals": MAX_GOALS, "grid": grid, "cutoff": args.cutoff},
        sharpe=None,
        notes=(f"H4: varredura de half-life por walk-forward. "
               f"RPS={winner['rps']:.5f} (n={winner['n_predictions']}), "
               f"rho_final={winner['final_rho']:.4f}. "
               f"Grade completa: "
               + "; ".join(f"{s['half_life']}d={s['rps']:.5f}" for s in sweep)),
        metric="rps")
    log.info("trial '%s' registrada em %s (N=%d tentativas no cofre)",
             TRIAL_NAME, TRIALS, len(registry.load()))
    return 0
