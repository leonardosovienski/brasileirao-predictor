...
def _hash(rows: list[dict[str, Any]]) -> str:
    payload = "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True, separators=(",", ":")) for row in rows).encode()
    return hashlib.sha256(payload).hexdigest()


def evaluate(picks_path: Path, results_path: Path, min_sample: int = 100) -> dict[str, Any]:
    picks = [json.loads(line) for line in picks_path.read_text(encoding="utf-8").splitlines() if line.strip()] if picks_path.exists() else []
    ...
    seen: set[Any] = set(); counts = {"emitted": len(picks), "eligible": 0, "rejected": 0, "matured": 0, "legacy_incomplete": 0}
    eligible: list[dict[str, Any]] = []; reasons: dict[str, int] = {}
    def reject(reason: str, legacy: bool = False) -> None:
        counts["rejected"] += 1; counts["legacy_incomplete" if legacy else "rejected"] += 0
    ...
    for pick in picks:
        ...
        counts["eligible"] += 1; eligible.append(merged)
    verdict = "INCONCLUSIVE"
    return {"schema_version": "shadow-cohort-evaluation/v1", "cohort": "H3/H5", "counts": counts, "classification": {"LEGACY_INCOMPLETE": counts["legacy_incomplete"], "PROSPECTIVE_ELIGIBLE": counts["eligible"], "MATURED_ELIGIBLE": counts["matured"], "PROSPECTIVE_REJECTED": max(0, counts["rejected"] - counts["legacy_incomplete"])}, "rejections": reasons, "dataset_hash": _hash(eligible), "min_sample": min_sample, "metrics": {"closing_coverage": sum(bool(r.get("closing_odds")) for r in eligible) / len(eligible) if eligible else 0.0}, "verdict": verdict, "capital_enabled": False}


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--picks", type=Path, default=ROOT / "data/sombra_picks.jsonl"); parser.add_argument("--results", type=Path, default=ROOT / "data/sombra_results.jsonl"); parser.add_argument("--output", type=Path)
    args = parser.parse_args(); report = evaluate(args.picks, args.results); encoded = json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n"
    if args.output: args.output.with_suffix(args.output.suffix + ".tmp").write_text(encoded, encoding="utf-8"); args.output.with_suffix(args.output.suffix + ".tmp").replace(args.output)
