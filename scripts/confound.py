...
def gp(lh, la):
    ...
    pa = nbinom.pmf(K, r, r / (r + lh))
    pb = nbinom.pmf(K, r, r / (r + la))
    g = np.outer(pa, pb)
    g[0, 0] *= 1 - lh * la * rho
    g[0, 1] *= 1 + lh * rho
    g[1, 0] *= 1 + la * rho
...
def prep(win):
    tm = sorted(set([g[1] for g in win]) | set([g[2] for g in win]))
    idx = {t: i for i, t in enumerate(tm)}
    N = len(tm)
    hi = np.array([idx[g[1]] for g in win])
    ai = np.array([idx[g[2]] for g in win])
    nn = np.array([0.0 if g[6] else 1.0 for g in win])
    hg = np.array([g[3] for g in win], float)
    ag = np.array([g[4] for g in win], float)
    return tm, idx, N, hi, ai, nn, hg, ag


def fit_maher(win, lr):
    tm, idx, N, hi, ai, nn, hg, ag = prep(win)
    ...
    def fg(p):
        ...
        atk = p[2 : 2 + N]
        dfn = p[2 + N : 2 + 2 * N]
        lph = mu + atk[hi] - dfn[ai] + gam * nn
        lpa = mu + atk[ai] - dfn[hi]
        lh = np.exp(lph)
        la = np.exp(lpa)
        nll = float(np.sum(lh - hg * lph + la - ag * lpa) + lr * (np.sum(atk**2) + np.sum(dfn**2)))
        rh = lh - hg
        ra = la - ag
        gat = np.zeros(N)
        np.add.at(gat, hi, rh)
    ...
    r = minimize(fg, np.r_[mu0, 0.0, np.zeros(2 * N)], jac=True, method="L-BFGS-B")
    return (
        "M",
        r.x[0],
        r.x[1],
        dict(zip(tm, r.x[2 : 2 + N])),
        dict(zip(tm, r.x[2 + N : 2 + 2 * N])),
    )
...
def fit_str(win, lr):
    ...
    def fg(p):
        mu, gam = p[0], p[1]
        s = p[2 : 2 + N]
        lph = mu + s[hi] - s[ai] + gam * nn
        lpa = mu + s[ai] - s[hi]
        lh = np.exp(lph)
        la = np.exp(lpa)
        nll = float(np.sum(lh - hg * lph + la - ag * lpa) + lr * np.sum(s**2))
        rh = lh - hg
        ra = la - ag
        gs = np.zeros(N)
    ...
    r = minimize(fg, np.r_[mu0, 0.0, np.zeros(N)], jac=True, method="L-BFGS-B")
    return ("S", r.x[0], r.x[1], dict(zip(tm, r.x[2 : 2 + N])), None)


def lam(par, g):
    typ, mu, gam, d1, d2 = par
    if typ == "M":
        return math.exp(mu + d1.get(g[1], 0) - d2.get(g[2], 0) + (0 if g[6] else gam)), math.exp(
            mu + d1.get(g[2], 0) - d2.get(g[1], 0)
        )
    return math.exp(mu + d1.get(g[1], 0) - d1.get(g[2], 0) + (0 if g[6] else gam)), math.exp(
        mu + d1.get(g[2], 0) - d1.get(g[1], 0)
    )
...
t0 = "2024-01-01"
tv = (date.fromisoformat(t0) - timedelta(days=183)).isoformat()
wtr = window(tv)
wv = [r for r in rows if tv <= r[0] < t0]
...
def tune(fitf):
    ...
    for lr in (1, 3, 10, 30, 100):
        par = fitf(wtr, lr)
        dev = sum(
            (lam(par, g)[0] - g[3] * math.log(max(lam(par, g)[0], 1e-9)))
            + (lam(par, g)[1] - g[4] * math.log(max(lam(par, g)[1], 1e-9)))
            for g in wv
        )
        if dev < best[1]:
            best = (lr, dev)
...
LRm = tune(fit_maher)
LRs = tune(fit_str)
print(f"lam_reg: Maher={LRm} | Forca-unica={LRs}\n")
months = []
y, m = 2024, 1
while (y, m) <= (2026, 5):
    months.append((y, m))
...
def me(y, m):
    return ms(y + 1, 1) if m == 12 else ms(y, m + 1)
...
for y, m in months:
    s, e = ms(y, m), me(y, m)
    pm = fit_maher(window(s), LRm)
    ps = fit_str(window(s), LRs)
    for i, r in enumerate(rows):
        if not (s <= r[0] < e):
            continue
        y3 = 0 if r[3] > r[4] else (1 if r[3] == r[4] else 2)
        d = hist[i][0] / 400.0
        pE = gp(math.exp(a_b + b_b * d), math.exp(a_b - b_b * d))
        pS = gp(*lam(ps, r))
        pM = gp(*lam(pm, r))
        B["elo"].append(brier(pE, y3))
        B["str"].append(brier(pS, y3))
        B["mah"].append(brier(pM, y3))
        obs.append(y3)
...
def pair(x, y):
    ...
    return d.mean(), d.std(ddof=1) / math.sqrt(len(d))


mc, sc = pair(B["str"], B["mah"])
gain = "REAL" if mc / sc > 2 else "NAO significativo"
print(f"\n  forca-unica - Maher: {mc:+.4f} (SE {sc:.4f}) -> ganho de atk/def {gain}")
ec, esc = pair(B["elo"], B["str"])
cadence = "real" if abs(ec / esc) > 2 else "desprezivel"
print(f"  Elo - forca-unica  : {ec:+.4f} (SE {esc:.4f}) -> ganho de cadencia/batch {cadence}")
