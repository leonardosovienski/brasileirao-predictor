...
def per_game_rps(results: list[dict[str, Any]]) -> list[float]:
    ...
    for r in results:
        ...
        out.append(rps([[v["home"], v["draw"], v["away"]]],
                       [outcome_index(r["actual"])]))
    return out
    ...
    for a, b in zip(dc, elo):
        ...
        if (ma["home"], ma["away"]) != (mb["home"], mb["away"]):
            raise AssertionError(
                f"confronto divergente no index {a['index']}: "
                f"{ma['home']}x{ma['away']} vs {mb['home']}x{mb['away']}")


def paired_bootstrap_ci(delta: "np.ndarray", *, n_boot: int = N_BOOT,
...
def main() -> int:
    ...
    dc_ev = BrasileiraoDixonColesEvaluator(half_life_days=float(HALF_LIFE_WINNER),
                                           max_goals=MAX_GOALS)
    dc_results = dc_ev.run(observations, min_history=MIN_HISTORY,
                           retrain_every=RETRAIN_EVERY)
    log.info("braço H4: %d previsões em %.0fs", len(dc_results),
    ...
    elo_results = elo_ev.run(observations, min_history=MIN_HISTORY,
                             retrain_every=1)
    log.info("braço H0: %d previsões em %.0fs", len(elo_results),
             time.perf_counter() - t0)
    ...
    log.info("pareado: n=%d | RPS médio DC=%.5f | Elo=%.5f | ΔRPS médio=%.5f",
             delta.shape[0], rps_dc.mean(), rps_elo.mean(), delta.mean())

    # --- bootstrap ------------------------------------------------------------
    lo, hi = paired_bootstrap_ci(delta)
    log.info("bootstrap %d reamostras (seed=%d): IC95%% de ΔRPS = [%.5f, %.5f]",
             N_BOOT, SEED, lo, hi)
    ...
    if hi < 0:
        ...
        detail = ("IC 95% cruza zero — a redução de erro sobre o Elo puro é "
                  "indistinguível de sorte" if lo < 0 else
                  "IC 95% estritamente positivo — o DC erra MAIS que o Elo puro")
    ...
    registry.register(
        ...
        notes=(existing.get("notes", "") +
               f" | VEREDITO ({date.today().isoformat()}): {verdict_txt} vs Elo puro — "
