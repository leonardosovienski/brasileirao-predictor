...
for row in conn.execute(
        ...
        "odds_over, odds_under FROM sofascore_matches").fetchall():
...
for i in test_idx:
    ...
    for side, edge, model_p, mkt_p in (("OVER", ev_over, p_over_model, p_over_mkt),
...
print("=== OU2.5 — apostas dentro da janela EV [%.0f%%, %.0f%%] ===" % (MIN_EDGE * 100, MAX_EDGE * 100))
print(f"{'Data':10} {'Casa':18} {'Fora':18} {'Lado':6} {'EV':7} {'P(mod)':7} {'P(mkt)':7} {'Odd':6} {'Ganhou':7} {'P&L'}")
for d, h, a, side, edge, mp, kp, odd, won, pnl in ou_bets:
    print(f"{d:10} {h:18} {a:18} {side:6} {edge:+.1%}  {mp:.2f}   {kp:.2f}   {odd:<6} {'sim' if won else 'nao':7} {pnl:+.2f}")
n_ou = len(ou_bets)
wins_ou = sum(1 for b in ou_bets if b[8])
pnl_ou = sum(b[9] for b in ou_bets)
print(f"\nOU2.5: {n_ou} apostas geradas | {wins_ou} vitorias ({wins_ou/n_ou:.1%})" if n_ou else "\nOU2.5: nenhuma aposta na janela de EV")
if n_ou:
...
print(f"\nDC: {n_dc} apostas geradas | {wins_dc} vitorias ({wins_dc/n_dc:.1%})" if n_dc else "\nDC: nenhuma aposta na janela de EV")
if n_dc:
