...
def evaluate(year):
    ...
    for i in idx:
        ...
        p = model.predict_match(history[i][0], 0.0, params_cache[mkey], 0.0,
                                max_goals=MAXG)
        y = 0 if hs > as_ else (1 if hs == as_ else 2)
        rec = {
            "date": d, "home": h, "away": a, "hs": hs, "as": as_,
            "y": y, "pm": (p["p_win"], p["p_draw"], p["p_loss"]),
            "lam_h": p["lambda_a"], "lam_a": p["lambda_b"],
            "p_over25": p["over"][2.5], "total_real": hs + as_,
            "diff": history[i][0],
        }
        o = odds.get((d, h, a))
...
def report(tag, recs):
    ...
    print(f"empates: previstos como pick {draws_pred} | reais {draws_real} "
          f"({draws_real/n:.0%}) | P(empate) media {st.mean(r['pm'][1] for r in recs):.1%}")
    print(f"nitidez: probMax media {pmax:.1%} | lambda_total media {st.mean(tot):.2f} "
    ...
    if wm:
        ...
        print(f"vs MERCADO (fechamento Shin, n={len(wm)}): "
              f"Brier modelo {mbr:.4f} | mercado {kbr:.4f} (dif {mbr-kbr:+.4f})")
        print(f"  acerto modelo {macc:.1%} | mercado {kacc:.1%} | "
              f"probMax mercado {kpmax:.1%}")
    wo = [r for r in recs if "mk_over" in r]
    if wo:
        oacc = st.mean(int((r["p_over25"] > .5) == (r["total_real"] > 2.5)) for r in wo)
        obr = st.mean((r["p_over25"] - (r["total_real"] > 2.5)) ** 2 for r in wo)
        kbr2 = st.mean((r["mk_over"] - (r["total_real"] > 2.5)) ** 2 for r in wo)
        kacc2 = st.mean(int((r["mk_over"] > .5) == (r["total_real"] > 2.5)) for r in wo)
        over_rate = st.mean(int(r["total_real"] > 2.5) for r in wo)
        print(f"OU2.5 (n={len(wo)}): acerto modelo {oacc:.1%} | mercado {kacc2:.1%} | "
              f"Brier modelo {obr:.4f} | mercado {kbr2:.4f} | taxa over real {over_rate:.1%}")
        print(f"  P(over) media modelo {st.mean(r['p_over25'] for r in wo):.1%} | "
...
def team_bias(recs, tag):
    ...
    atk = defaultdict(float); dfs = defaultdict(float); nj = defaultdict(int)
    for r in recs:
        atk[r["home"]] += r["hs"] - r["lam_h"]; dfs[r["home"]] += r["lam_a"] - r["as"]
        atk[r["away"]] += r["as"] - r["lam_a"]; dfs[r["away"]] += r["lam_h"] - r["hs"]
        nj[r["home"]] += 1; nj[r["away"]] += 1
    print(f"\n--- vies por time ({tag}): dAtq = gols feitos - esperados; "
          f"dDef = esperados - sofridos ---")
...
PROMOVIDOS = {"2025": ["Ceará", "Mirassol", "Santos", "Sport Recife"],
...
for yr, teams, recs in (("2025", PROMOVIDOS["2025"], r25), ("2026", PROMOVIDOS["2026"], r26)):
    ...
    if not sub:
        continue
    pts = 0.0; exp_pts = 0.0; n = 0
    for r in sub:
        for side, ph, pd in ((0, r["pm"][0], r["pm"][1]),):
            pass
        for t in teams:
            if r["home"] == t:
                won = r["y"] == 0; drew = r["y"] == 1
                p_w, p_d = r["pm"][0], r["pm"][1]
            elif r["away"] == t:
                won = r["y"] == 2; drew = r["y"] == 1
            ...
            exp_pts += 3 * p_w + p_d
            n += 1
    print(f"  {yr}: {n} jogos-time | pontos reais {pts:.0f} | esperados pelo modelo "
