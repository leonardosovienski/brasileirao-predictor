...
for lo, hi in bins:
    ...
    lt = st.mean(d[1] for d in g)
    ld = st.mean(d[2] for d in g)
    pdr = st.mean(d[3] for d in g)
...
g0 = [d for d in data if d[0] < 50]
g1 = [d for d in data if d[0] >= 200]
...
print("observado, a baixa variancia e da FUNCAO (cosh), nao dos dados. O link so")
print("REDISTRIBUI gols (lambda_diff), nunca muda o TOTAL esperado por matchup.")
