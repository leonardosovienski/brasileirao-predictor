...
Uso: python scripts/backtest_close.py
"""
...
spec = importlib.util.spec_from_file_location(
    "backtest_walkforward", ROOT / "scripts" / "backtest_walkforward.py")
...
def _settle_close(market, selection, p_model, p_shin_close, odd_open,
                  odd_close, won, ctx, min_edge, max_edge):
    """Descarta a abertura: gatilho e P&L sempre no fechamento."""
    return _settle_orig(market, selection, p_model, p_shin_close, None,
                        odd_close, won, ctx, min_edge, max_edge)


# patch nos DOIS namespaces: o walkforward importou o nome, e o
# _settle_extended resolve dentro de src.backtest
...
def main():
    ...
    for mkt in sorted({b["market"] for b in ledger}):
        ...
        print(f"  {mkt:<12} n={len(bets):<5} acerto "
              f"{st.mean(b['won'] for b in bets):.1%}  ROI {st.mean(pnl):+.1%}")
    ...
    if len(returns) >= 30:
        ...
        print(f"  ROI {st.mean(returns):+.1%} | acerto "
              f"{st.mean(b['won'] for b in h1):.1%} | sharpe/aposta {sr:.4f}")
        print(f"  PSR {psr:.2f} | IC95 pnl médio (cluster) [{lo:+.4f}, {hi:+.4f}]")
        edge_pos = lo is not None and lo > 0
        print(f"  -> edge a fechamento: "
              f"{'POSITIVO com IC fechado' if edge_pos else 'NÃO demonstrado'}")
        print("\n  referência (backtest oficial, preço de ABERTURA): "
              "n=455, ROI +7,9%, PSR 0,94, IC95 [-0,022, +0,172]")
    else:
        print("  amostra insuficiente")

    print("\n(diagnóstico read-only — nenhum trial registrado, nenhum artefato "
          "oficial sobrescrito)")
