...
Uso: python scripts/sim_melhorias.py [--valida]
"""
import math
import os
import sys
...
rows = conn.execute(
    ...
    "FROM matches WHERE home_score IS NOT NULL ORDER BY date, home_team").fetchall()
...
def fit_batch(train, fit_date, half_life_years, reg, atk_def):
    ...
    w = np.array([0.5 ** ((fd - date.fromisoformat(r[0])).days / hl_days)
                  for r in train])

    nstr = 2 * T if atk_def else T
    # x = [mu, ha, strengths..., log_alpha, rho]
    x0 = np.r_[math.log(max(np.r_[hs, as_].mean(), 1e-3)), 0.25,
               np.zeros(nstr), math.log(0.1), -0.03]
    ...
    bounds = ([(-3, 3), (-1, 1)] + [(-2.5, 2.5)] * nstr
              + [(math.log(1e-4), math.log(3)), (-0.35, 0.35)])
    res = minimize(negll, x0, method="L-BFGS-B", bounds=bounds,
                   options={"maxiter": 800})
    x = res.x
    out = {"mu": x[0], "ha": x[1], "log_alpha": x[2 + nstr], "rho": x[3 + nstr],
           "teams": tix, "atk_def": atk_def, "ok": bool(res.success)}
    if atk_def:
        out["atk"] = x[2:2 + T]
...
def predict_batch(p, home, away):
    ...
    return {"pm": (g["p_win"], g["p_draw"], g["p_loss"]),
            "p_over25": g["over"][2.5], "lam_h": lh, "lam_a": la}
...
def fit_batch_xg(train, fit_date, half_life_years, reg, w_xg):
    ...
    r1 = minimize(negll1, x0, method="L-BFGS-B", bounds=b1,
                  options={"maxiter": 800})
    mu, ha = r1.x[0], r1.x[1]
    ...
    r2 = minimize(negll2, [math.log(0.1), -0.03], method="L-BFGS-B",
                  bounds=[(math.log(1e-4), math.log(3)), (-0.35, 0.35)])
    return {"mu": mu, "ha": ha, "atk": atk, "def": dfn, "teams": tix,
            "atk_def": True, "log_alpha": r2.x[0], "rho": r2.x[1],
    ...
    for i in idx:
        ...
        if mkey not in cache:
            if model_kind == "baseline":
                ...
                train = [(r[0], r[1], r[2], r[3], r[4])
                         for r in rows if r[0] < d]
                if model_kind.startswith("xg"):
                    w_xg = float(model_kind.split(":")[1])
                    cache[mkey] = fit_batch_xg(train, d, hl, reg, w_xg)
                else:
                    cache[mkey] = fit_batch(train, d, hl, reg,
                                            atk_def=(model_kind == "atkdef"))
        p = cache[mkey]
        if model_kind == "baseline":
            r = model.predict_match(elo_history[i][0], 0.0, p, 0.0, max_goals=MAXG)
            pred = {"pm": (r["p_win"], r["p_draw"], r["p_loss"]),
                    "p_over25": r["over"][2.5], "lam_h": r["lambda_a"],
                    "lam_a": r["lambda_b"]}
        else:
            pred = predict_batch(p, h, a)
        y = 0 if hs > as_ else (1 if hs == as_ else 2)
        rec = {"date": d, "home": h, "away": a, "hs": hs, "as": as_, "y": y,
               "over_real": int(hs + as_ > 2.5), **pred}
        o = odds.get((d, h, a))
...
def summarize(tag, recs):
    ...
    oacc = st.mean(int((r["p_over25"] > .5) == r["over_real"]) for r in recs)
    pmax = st.mean(max(r["pm"]) for r in recs)
    tot = [r["lam_h"] + r["lam_a"] for r in recs]
    print(f"{tag:28s} Brier {br:.4f} | LL {ll:.4f} | acc {acc:.1%} | "
          f"BrierOU {ob:.4f} | accOU {oacc:.1%} | probMax {pmax:.1%} | "
          f"lamTot {st.mean(tot):.2f}±{st.pstdev(tot):.2f}")
    return br


def paired_bootstrap(base, cand, key=lambda r: brier3(r["pm"], r["y"]),
                     n_iter=2000, seed=13):
...
if "--valida" in sys.argv:
    ...
    tp = lambda d: "2024-08-01" <= d < "2025-01-01"
...
def compara(base, recs, nome):
    ...
    m2, lo2, hi2 = paired_bootstrap(
        base, recs, key=lambda r: (r["p_over25"] - r["over_real"]) ** 2)
...
for periodo in ("2025", "2026", "2025+2026"):
    tp = (lambda d: d.startswith("2025") or d.startswith("2026")) \
        if periodo == "2025+2026" else (lambda d, y=periodo: d.startswith(y))
    print(f"\n== teste {periodo} ==")
    base = walkforward(tp, "baseline")
    summarize("baseline (Elo+cosh)", base)
    mrecs = [r for r in base if "mk" in r]
    if mrecs:
        print(f"{'mercado (Shin fechamento)':28s} Brier "
    ...
    compara(base, [ensemble(b, c) for b, c in zip(base, c3)],
            "C4 ensemble 50/50 base+C3")
