"""Research-only cached-parameter NB x NB + four-cell DC, with an explicit truncation bound.
...
def parameters(home,away,alpha,rho):
    ...
    if not(h>0 and a>0 and 1e-4<=d<=3):raise ValueError('outside research domain')
    ...
    if not (0<ph<1 and 0<pa<1):raise ValueError('unrepresentable NB probability')
...
def adaptive_grid(home,away,alpha,rho,*,mass_tolerance=1e-6,max_goals=512):
    ...
    def diagnostic(k):
        ...
        sh=float(nbinom.sf(k,r,ph));sa=float(nbinom.sf(k,r,pa))
        raw_tail=sh+sa-sh*sa
        return {'max_goals':int(k),'dc_total_mass':z,'raw_tail_mass':raw_tail,
                'raw_retained_mass':z-raw_tail,'retained_probability_before_conditional_renormalization':1-raw_tail/z,
                'omitted_mass':raw_tail/z}
