...
## Reabertura da auditoria dependente de dados — execução operacional 8a3a2415

Foi criada a família `reports/ou25_v2/ou25_operational_8a3a2415_2026-08-28/`, sem sobrescrever os nested históricos. A base operacional usada foi a cópia somente-leitura com SHA-256 `8a3a2415aab9b8525708ee18ee7b3fb360b40031904095f5c71b35871e5946cd`; a base não corresponde a `5f6c35df...` nem a `f070ce13...`.

A base contém 141 fixtures com data posterior ao cutoff de 2026-08-28; todas as 141 são não finalizadas e nenhuma fixture futura está finalizada. O loader do replay seleciona somente partidas com ambos os placares presentes, portanto essas fixtures não entram na avaliação. Há 2.125 partidas finalizadas e 156 não finalizadas. Não foram encontradas duplicatas nas chaves principais de `matches`, duplicatas de `event_id` em `matches` ou `sofascore_matches`, nem chaves estrangeiras órfãs.

A auditoria temporal identificou uma limitação que passou a ser tratada como bloqueio econômico: `sofascore_matches` fornece odds agregadas sem `bookmaker` nominal e sem `captured_at`. Os 200 snapshots pré-jogo têm 68 linhas OU2.5, formando dez eventos completos, todos com timestamps anteriores ao kickoff e odds dentro dos limites; contudo, a ausência de bookmaker impede atribuir esses snapshots a uma cotação executável/closing compatível. Os demais 24.942 snapshots são `pre_match=0` e não podem ser tratados como preços executáveis.

O replay exploratório inicial, gerado antes do novo guard, produziu 229 picks e 51,30 unidades, mas é explicitamente inválido para uso econômico: 203 picks pertencem às temporadas observadas/contaminadas de 2024–2026 e os preços vieram de campos agregados de `sofascore_matches`; apenas 26 picks não contaminados são de 2023, todos Under, com 7,85 unidades. Esses números são diagnóstico histórico, não evidência de vantagem. O resultado não foi promovido, e o candidato permanece nulo/`NO_BET`.

Para impedir a repetição do erro, o runner agora falha alto sem uma fonte explícita de preços point-in-time. A regressão correspondente passa, e a CI completa após a correção passou com 815 testes, 1 desmarcado e 3 warnings, incluindo predict, primeiro tempo e segundo tempo. O manifesto da nova família marca `validity=INVALID_FOR_ECONOMIC_USE`, `point_in_time_price_source=false` e `prospective_validation_eligible=false`.

A conclusão principal permanece: a infraestrutura e os smokes operacionais passam, mas não há replay econômico legítimo com bookmaker/timestamp/closing compatíveis nessa base. O resultado continua `NO_BET`, capital e Kelly desabilitados, força máxima 40 e nenhuma promoção para A1.
