            ...
            try:
                if verified and (await client.info("server"))["run_id"] == instance:
                    ...
                    if state.runs:
                        ...
                        await client.zrem(protocol.READY_KEY, *state.runs)
            finally:
                await client.aclose()
...
def test_completed_result_is_discoverable_after_ready_notification_is_lost():
    async def scenario(s):
        raw = s.request()
        await s.register(raw)
        # No fair_odds_ready subscriber exists during completion.
        await kernel_daemon._handle_invoke(s.client, raw, PARAMS)
        run = json.loads(raw)["run_id"]
        fair_key = "fair_odds:" + s.match
        assert await s.client.hget(protocol.REQUEST_PREFIX + run, "status") == b"completed"
        deadline = await s.client.zscore(protocol.READY_KEY, run)
        assert deadline is not None, "completed fair result must remain discoverable without Pub/Sub delivery"
        assert deadline == await s.client.pexpiretime(fair_key)
        original = await s.client.get(fair_key)
        await asyncio.sleep(0.01)
        await kernel_daemon._handle_invoke(s.client, raw, PARAMS)
        assert await s.client.get(fair_key) == original
        assert await s.client.pexpiretime(fair_key) == deadline
        assert await s.client.zscore(protocol.READY_KEY, run) == deadline

    run_scenario(scenario)


def test_completion_replay_removes_expired_ready_without_recreating_fair():
    async def scenario(s):
        raw = s.request()
        await s.register(raw)
        await kernel_daemon._handle_invoke(s.client, raw, PARAMS)
        run = json.loads(raw)["run_id"]
        assert await s.client.zscore(protocol.READY_KEY, run) is not None
        await s.client.pexpire("fair_odds:" + s.match, 1)
        await asyncio.sleep(0.01)
        assert await s.complete(raw, "old-completed-token") == b"COMPLETED"
        assert await s.client.zscore(protocol.READY_KEY, run) is None
...
def test_old_completion_removes_only_its_ready_member():
    async def scenario(s):
        old, new = s.request(), s.request(version="2")
        await s.register(old)
        await kernel_daemon._handle_invoke(s.client, old, PARAMS)
        old_run, new_run = json.loads(old)["run_id"], json.loads(new)["run_id"]
        assert await s.client.zscore(protocol.READY_KEY, old_run) is not None
        # Deliberately leave the old ready entry to exercise the kernel fence;
        # normal .NET registration also removes its superseded ready entry.
        await s.register(new)
        await kernel_daemon._handle_invoke(s.client, new, PARAMS)
        deadline = await s.client.zscore(protocol.READY_KEY, new_run)
        assert await s.complete(old, "old-completed-token") == b"STALE"
        assert await s.client.zscore(protocol.READY_KEY, old_run) is None
        assert await s.client.zscore(protocol.READY_KEY, new_run) == deadline
        assert await s.client.pexpiretime("fair_odds:" + s.match) == deadline
        assert json.loads(await s.client.get("fair_odds:" + s.match))["run_id"] == new_run

    run_scenario(scenario)


def test_invalid_ready_index_type_is_rejected_before_fair_write(monkeypatch):
    async def scenario(s):
        raw = s.request()
        await s.register(raw)
        assert await s.claim(raw, "owner") == b"CLAIMED"
        bad_ready = "synthetic-invalid-ready:" + s.match
        s.keys.add(bad_ready)
        monkeypatch.setattr(protocol, "READY_KEY", bad_ready)
        await s.client.set(bad_ready, "wrong-type")
        assert await s.complete(raw, "owner") == b"INVALID_STATE"
        assert await s.client.get("fair_odds:" + s.match) is None
        run = json.loads(raw)["run_id"]
        assert await s.client.hget(protocol.REQUEST_PREFIX + run, "status") == b"pending"
        assert await s.client.get(protocol.LEASE_PREFIX + run) == b"owner"
...
@pytest.mark.parametrize("denied", ["channel", "hset", "zrem", "zadd", "pexpiretime", "del", "set", "publish"])
