...
public static class KernelRedisProtocolV2
{
    ...
    // KEYS: lineup, current, sequence, new request, identity, fair, pending, ready.
    ...
    public const string Register = Checks + "\n" + """
        ...
        kind(KEYS[4], 'hash'); kind(KEYS[5], 'string'); kind(KEYS[6], 'string'); kind(KEYS[7], 'zset'); kind(KEYS[8], 'zset')
    ...
    // KEYS: lineup, current, fair, pending, ready.
    ...
    public const string ApplyWatchdog = Checks + "\n" + """
        kind(KEYS[1], 'string'); kind(KEYS[2], 'string'); kind(KEYS[3], 'string'); kind(KEYS[4], 'zset'); kind(KEYS[5], 'zset')
        ttl(ARGV[5])
        cjson.decode(ARGV[4])
