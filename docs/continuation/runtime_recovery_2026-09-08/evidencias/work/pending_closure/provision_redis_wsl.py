...
BASE = Path(__file__).resolve().parent / "redis_env_pending_closure"
LOGS = BASE / "receipts"
DISTRO = "codex-brasileirao-pending-20260908"
...
def build_official_github():
    ...
    raw = download(url)
    tagged = download("https://codeload.github.com/redis/redis/tar.gz/refs/tags/8.2.1")
    with (BASE / "redis-github-8.2.1.tar.gz").open("xb") as handle:
        handle.write(tagged)
    def tree(blob):
