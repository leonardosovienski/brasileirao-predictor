...
def parse_match(ev: dict):
    ...
    if ts and ts > 1e11:  # Sofascore trocou a unidade pra ms: normaliza
        ...
        ts = int(ts // 1000)
    date = datetime.fromtimestamp(ts, UTC).strftime("%Y-%m-%d") if ts else None
...
def parse_xg(stats: dict | None):
    """Read full-match xG, preserving zeros and rejecting ambiguous payloads.

    Expected goals on target is a different statistic. Conflicting duplicates
    or malformed xG do not fall back to whichever item happens to appear first.
    A reported zero remains zero; a separate quality audit assesses provenance.
    """
    ...
    for period in periods:
        ...
        if not isinstance(groups, list):
            return None, None
        for group in groups:
            if not isinstance(group, dict) or not isinstance(group.get("statisticsItems", []), list):
                return None, None
            for item in group.get("statisticsItems", []):
                if not isinstance(item, dict):
                    return None, None
                name = item.get("name", "")
                if not isinstance(name, str) or name.strip().casefold() not in {
                    "expected goals",
                    "expected goals (xg)",
                }:
                    continue
                if "key" in item and item["key"] != "expectedGoals":
                    return None, None
                pair = []
...
def run(
    ...
    *,
    ...
    for comp in comps:
        ...
        total = len(events)
        log.info(
            "%s: %d jogos a coletar (~%.0fs com rate limit)...",
            name,
            total,
            total * client.rate * 2,
        )
        for i, ev in enumerate(events, 1):
            ...
            try:
                ...
                snaps = [
                    (eid, now, "1x2", sel, odd, int(pre))
                    for sel, odd in (("home", oh), ("draw", od), ("away", oa))
                    if odd
                ]
                snaps += [
                    (eid, now, f"ou{ou_line}", sel, odd, int(pre))
                    for sel, odd in (("over", o_over), ("under", o_under))
                    if odd
                ]
                if snaps:
                ...
                parsed_open = parse_all_odds(raw_odds, m["home_team"], m["away_team"], initial=True)
                ...
                n_matches += 1
                placar = f"{m['home_score']}x{m['away_score']}" if m["finished"] else "fixture"
                log.info("  [%d/%d] %s %s %s", i, total, m["home_team"], placar, m["away_team"])
            except Exception as e:
