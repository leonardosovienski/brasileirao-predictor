...
def test_xg_coverage_separates_paired_observation_from_legacy_fallback():
    ...
    assert report["total"]["played"] == 3
    assert report["total"]["paired_xg_valid"] == report["total"]["paired_xg_present"] == 1
    assert report["total"]["paired_coverage"] == 1 / 3
    assert report["total"]["numeric_nonzero_pair"] == 1
