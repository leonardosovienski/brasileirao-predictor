...
def test_superseded_conflicting_revisions_fail_closed_in_every_input_order(order) -> None:
    original = HISTORY[-1]
    versions = (
        original,
        replace(original, home_xg=5),
        replace(original, home_xg=4, available_at=DECISION - timedelta(seconds=1)),
    )
    with pytest.raises(ValueError, match="conflicting statistics"):
        predict([*HISTORY[:-1], *(versions[index] for index in order)])


@pytest.mark.parametrize("order", tuple(permutations(range(3))))
def test_identical_superseded_duplicates_are_tolerated_in_every_input_order(order) -> None:
    original = HISTORY[-1]
    latest = replace(original, home_xg=4, available_at=DECISION - timedelta(seconds=1))
    versions = (original, replace(original), latest)
    expected = predict([*HISTORY[:-1], latest])
    assert predict([*HISTORY[:-1], *(versions[index] for index in order)]) == expected


def test_conflicting_future_revisions_are_ignored_before_conflict_validation() -> None:
    future = replace(HISTORY[-1], home_xg=4, available_at=DECISION)
    conflict = replace(future, home_xg=5)
    for order in permutations((future, conflict)):
        assert predict([*order, *HISTORY]) == predict()
