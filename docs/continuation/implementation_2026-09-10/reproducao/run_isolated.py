...
def main():
    ...
    allowed_public = {
        (REPO / 'data/promotions_brasileirao_2018_2026.json').resolve(),
        (REPO / 'reports/trial_draw_calibration_a10_2024.json').resolve(),
    }
    ...
    def guard(event, args):
        ...
        if event == 'sqlite3.connect':
            db = str(args[0])
            target = unquote(db[5:].split('?')[0]) if db.startswith('file:') else db
        ...
        if event == 'open':
            ...
            if p not in allowed_public and not p.is_relative_to(out) and (p.name == '.env' or p.is_relative_to(Path('C:/BRASILEIRAO/DADOS_PRESERVADOS')) or any(p.is_relative_to(REPO / d) for d in ('data','reports','research'))):
