...
public sealed class MarketStateEngine : BackgroundService
{
                                            ...
                                            CancellationToken ct)
    {
        ...
        try
        {
            ...
            using var notification = JsonDocument.Parse(fairOddsJson);
            _ = FairOddsPayload.FromDict(notification.RootElement);
            var stored = doc.RootElement;
            var notified = notification.RootElement;
            if (stored.GetProperty("match_id").GetString() != matchId ||
                notified.GetProperty("match_id").GetString() != matchId ||
                stored.GetProperty("job_id").GetString() != notified.GetProperty("job_id").GetString() ||
                stored.GetProperty("run_id").GetString() != notified.GetProperty("run_id").GetString())
            {
    ...
    public async Task InvokeKernelAsync(
        ...
        double dvorpA, double dvorpB, string sourceEventId)
    {
        ...
        var invocationIdentity = JsonSerializer.SerializeToUtf8Bytes(
            new { sourceEventId, matchId, eloA, eloB, dvorpA, dvorpB });
