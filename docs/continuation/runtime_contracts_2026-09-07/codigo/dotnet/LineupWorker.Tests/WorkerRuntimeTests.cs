...
public sealed class WorkerRuntimeTests : IAsyncLifetime
{
    ...
    public async Task MarketStateInvokesVersionedKernelAndProcessesFairOdds()
    {
        ...
        await mse.InvokeKernelAsync("m-edge", 1600, 1500, 0.1, -0.1, "synthetic-lineup-event");
    ...
    public async Task WorkerConsumesBothLineupsPersistsStateAndInvokesKernel()
    {
        ...
        await sub.SubscribeAsync(RedisChannel.Literal("system:invoke_kernel"), (_, value) =>
            invocations.Writer.TryWrite(JsonSerializer.Deserialize<KernelInvokePayload>(value.ToString())!));
        ...
        await sub.UnsubscribeAsync(RedisChannel.Literal("system:invoke_kernel"));
    }
