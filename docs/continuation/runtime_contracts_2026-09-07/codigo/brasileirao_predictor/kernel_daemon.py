...
_INVOKE_FIELDS = frozenset((*_INVOKE_ID_FIELDS, *_INVOKE_NUMBER_FIELDS, "protocol_version", "timestamp_t3"))


def _parse_invoke(payload_bytes: bytes) -> dict:
    """Valida redis-protocol-v1 sem I/O no hot path nem coerção de tipos JSON."""
    ...
    if msg["protocol_version"] != "brasileirao.redis/1":
...
async def _handle_invoke(client, payload_bytes: bytes, params: tuple) -> None:
    ...
    try:
        ...
        log.error("[kernel] payload inválido para redis-protocol-v1")
        return
