...
changes=sorted(set(git('diff','HEAD','--name-only').splitlines()+git('ls-files','--others','--exclude-standard').splitlines()+[p.relative_to(repo).as_posix() for p in docs.rglob('*') if p.is_file()]))
...
subprocess.run(['git','add','-f','--',*changes],cwd=repo,check=True)
subprocess.run(['git','diff','--cached','--check'],cwd=repo,check=True)
evidence_checks=[]
for path in ev.rglob('*'):
    if not path.is_file():continue
    rel=path.relative_to(repo).as_posix()
    indexed=subprocess.check_output(['git','show',':'+rel],cwd=repo)
    assert indexed==path.read_bytes(),rel
    evidence_checks.append(rel)
dump(root/'index-evidence-check.json',dict(files=evidence_checks,byte_equality=True))
