...
out = base / 'ENTREGAS/BRASILEIRAO_CLO_20260910'
bundle = base / 'BACKUPS/brasileirao-predictor-CLO-20260910.bundle'
archive = base / 'ENTREGAS/BRASILEIRAO_CLO_20260910_entrega.zip'
restore = root / 'restored-git.git'
audit = base / 'AUDITORIA/VERIFICACAO_ADICIONAL_2026-09-10.json'
...
for name in ['run_isolated.py', 'check_changed.py', 'integrate_tests.py', 'write_docs.py',
             'prepare_integration.py', 'prepare_integration-first.py', 'build_package.py',
             'backup_delivery.py', 'precommit.diff', 'staged-paths.json',
             'index-evidence-check.json', 'integration-test-scope.json', 'pyright.json']:
    copy(root / name, Path('reproduction') / name)
copy(base / 'LEIA_PRIMEIRO.md', Path('LEIA_PRIMEIRO.md'))
copy(base / 'INSTRUCOES/PROXIMO_PROMPT_APOS_VERIFICACAO_ADICIONAL_2026-09-10.md', Path('PROXIMO_PROMPT.md'))
...
receipt = dict(
    round='CLO-20260910', verified_at=datetime.now(UTC).isoformat(), commit=commit,
