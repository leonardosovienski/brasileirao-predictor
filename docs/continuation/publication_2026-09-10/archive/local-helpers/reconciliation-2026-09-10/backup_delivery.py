...
out = base / 'ENTREGAS/BRASILEIRAO_RCA_20260910'
bundle = base / 'BACKUPS/brasileirao-predictor-RCA-20260910.bundle'
archive = base / 'ENTREGAS/BRASILEIRAO_RCA_20260910_entrega.zip'
restore = root / 'restored-git.git'
audit = base / 'AUDITORIA/CONCILIACAO_MANDATO_2026-09-10.json'
...
for path in (root / 'package-after-guides/dist').iterdir():
    copy(path, Path('dist') / path.name)
for name in ['run_isolated.py', 'check_changed.py', 'write_docs.py',
             'prepare_integration.py', 'build_package.py',
...
copy(root / 'finalize_staging.py', Path('reproduction/finalize_staging.py'))
copy(base / 'LEIA_PRIMEIRO.md', Path('LEIA_PRIMEIRO.md'))
copy(base / 'INSTRUCOES/PROXIMO_PROMPT_APOS_CONCILIACAO_2026-09-10.md', Path('PROXIMO_PROMPT.md'))
...
receipt = dict(
    round='RCA-20260910', verified_at=datetime.now(UTC).isoformat(), commit=commit,
