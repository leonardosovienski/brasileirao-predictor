...
def main():
    ...
    def guard(event, args):
        ...
        if event == "open" and isinstance(args[0], str | bytes | os.PathLike):
            ...
            if (
                path.is_relative_to(repository / "data")
                or path.is_relative_to(Path("C:/BRASILEIRAO/DADOS_PRESERVADOS"))
                or path.name == ".env"
            ):
    ...
    raise SystemExit(
        pytest.main(
            [
                ...
                str(repository / "tests/test_historical_admission.py"),
                str(repository / "tests/test_historical_numeric_boundary.py"),
                str(repository / "tests/test_live_capture_admission.py"),
                str(repository / "tests/test_closing_scenario.py"),
