...
out=root/'package-runtime';out.mkdir(exist_ok=False)
...
def run(name,args,cwd=out,expected=0):
    ...
    (root/'package-receipt-runtime.json').write_text(json.dumps(dict(commands=records),indent=2),encoding='utf-8')
...
(root/'package-receipt-runtime.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
