...
out=root/'package-runtime-02';out.mkdir(exist_ok=False)
...
def run(name,args,cwd=out,expected=0):
    ...
    (root/'package-receipt-runtime-02.json').write_text(json.dumps(dict(commands=records),indent=2),encoding='utf-8')
...
run('build',[uv,'build','--wheel','--offline','--no-python-downloads','--python',python,'--out-dir',out/'dist'],cwd=repo)
...
(root/'package-receipt-runtime-02.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
