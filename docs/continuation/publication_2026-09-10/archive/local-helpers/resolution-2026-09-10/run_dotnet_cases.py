...
def main():
    ...
    env.update(
        ...
        PYTHONPATH=os.pathsep.join([str(Path('C:/BRASILEIRAO/brasileirao-predictor/tools/runtime_lab')), str(repo)]),
        OMP_NUM_THREADS="2",
    ...
    with (out / "redis.log").open("wb") as log:
        ...
        try:
            ...
            if False:  # explicit .NET-only diagnosis, no unrelated Python suite
            ...
            env.update(
                ...
                LINEUP_E2E_KERNEL_SCRIPT=str(Path('C:/BRASILEIRAO/brasileirao-predictor/tools/runtime_lab/kernel_synthetic.py')),
            )
