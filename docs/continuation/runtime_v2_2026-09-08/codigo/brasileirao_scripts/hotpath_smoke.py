...
def register_synthetic_lineup(client, *, timeout: float = 10.0) -> str:
    ...
    for side in ("home", "away"):
        ...
        while client.publish("lineups:synthetic-smoke", encoded) == 0:
            if time.perf_counter() >= deadline:
                raise TimeoutError("no Worker subscribed to synthetic lineup events")
            time.sleep(0.05)
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (redis.RedisError, ValueError, TypeError, KeyError, TimeoutError):
