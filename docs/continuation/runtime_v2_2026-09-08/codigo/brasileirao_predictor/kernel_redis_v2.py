...
# KEYS current, request, lease, fair, pending, lineup;
# ARGV payload, run, token, fair_json, fair_ms, channel, retry_ms.
COMPLETE_SCRIPT = (
    "-- kernel-v2:complete\n"
    + _HELPERS
    + """
if not allowed(KEYS[1], 'string') or not allowed(KEYS[2], 'hash')
    ...
    or not allowed(KEYS[5], 'zset') or not allowed(KEYS[6], 'string') then return 'INVALID_STATE' end
local fair_ms, retry_ms = tonumber(ARGV[5]), tonumber(ARGV[7])
if not fair_ms or fair_ms < 1 or fair_ms > 5000
    or not retry_ms or retry_ms < 1 or retry_ms > 5000
...
if not redis.acl_check_cmd('SET', KEYS[4], ARGV[4], 'PX', fair_ms)
    ...
    or not redis.acl_check_cmd('ZADD', KEYS[5], 0, ARGV[2])
    or not redis.acl_check_cmd('DEL', KEYS[3])
    or not redis.acl_check_cmd('DEL', KEYS[4]) then return 'ACL_DENIED' end
local stored = redis.call('HGET', KEYS[2], 'payload')
...
-- All expected argument/key errors are checked before the write. PUBLISH can
-- still fail (e.g. channel ACL); a pcall keeps that work pending and retryable.
redis.call('SET', KEYS[4], ARGV[4], 'PX', math.min(fair_ms, remaining))
local published = redis.pcall('PUBLISH', ARGV[6], ARGV[4])
if type(published) == 'table' and published.err then
    redis.call('DEL', KEYS[4])
    redis.call('DEL', KEYS[3])
    redis.call('ZADD', KEYS[5], now_ms() + retry_ms, ARGV[2])
    return 'RETRY'
end
